...
/// Exact terminal history for one managed ID absent from the OPEN scan.
#[derive(Debug)]
...
impl TerminalHistories {
    ...
    /// Applies the compatibility publisher's retirement policy before any
    /// write.
    ///
    /// Terminal history preserves what GitHub returned without assigning that
    /// evidence a policy. The compatibility publisher treats a closed or
    /// merged pull request as a terminal error, so its adapter validates that
    /// policy while the attempt is still read-only.
    pub(super) fn ensure_legacy_pushable(&self) -> Result<()> {
...
mod tests {
    ...
    #[test]
    fn legacy_retired_diagnostics_are_complete_and_deterministic() {
        let g1 = id("G1");
        let g2 = id("G2");
