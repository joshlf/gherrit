...
/// A fully prepared, finite input held by a read-only GHerrit-owned handle.
///
/// The file is unlinked and GHerrit retains no writable handle. The child is
/// deliberately trusted, however: this type does not make the underlying inode
/// immutable or sandbox a same-user child which seeks another writable alias.
/// It proves that no GHerrit-owned writer or input producer can extend the
/// command lifetime, and that the handle inherited as standard input is
/// read-only.
...
impl fmt::Display for CommandError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::StdinChanged => {
                formatter.write_str("remote Git command stdin changed while it was being prepared")
            }
