...
//!
//! Unix children start as process-group leaders and retain an armed bounded
//! best-effort group/leader kill-and-reap fallback until their leader is reaped
//! or wait no longer proves that its numeric identity is retained. Windows
//! children start suspended, receive a direct-child guard, and join a private
//! kill-on-close job before any fallible thread discovery or resume. Ordinary
//! paths perform bounded termination, boundary observation, reaping, and pipe
//! draining; drop guards cover panic and future-cancellation unwinding.
//!
...
///
/// Input and retained output have the same finite budget. The regular file is
/// fully prepared before execution, so neither its producer nor pipe capacity
/// can extend the command lifetime.
#[allow(dead_code, reason = "regular-file input is an opt-in command capability")]
...
///
/// The input is validated and rewound inside the blocking worker before the
/// child starts. This deliberately does not admit an arbitrary [`Stdio`],
/// pipe, or in-memory producer: all input is finite and complete before the
/// command deadline begins, with no producer lifetime to supervise.
#[allow(dead_code, reason = "regular-file input is an opt-in command capability")]
...
/// remains which could race the worker's final size validation.
#[allow(dead_code, reason = "regular-file input is an opt-in command capability")]
...
impl RegularFileStdinBuilder {
    ...
    /// Flushes the file and consumes its only caller-side writable handle.
