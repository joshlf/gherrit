...
use std::{
    os::unix::{
        ...
        process::{CommandExt as _, ExitStatusExt},
    },
...
fn reexec_helper() {
    ...
    match mode.as_str() {
        ...
        #[cfg(unix)]
        "terminal-prompt" => {
            let mut terminal = wait_for_foreground_terminal();
            terminal.write_all(b"TERMINAL PROMPT\n").unwrap();
            terminal.flush().unwrap();
            let mut response = [0; 3];
            terminal.read_exact(&mut response).unwrap();
            assert_eq!(&response, b"ok\n");
            process::exit(reexec_bytes() as i32);
        }
        #[cfg(unix)]
        "terminal-wait" => {
            let mut terminal = wait_for_foreground_terminal();
            ...
            terminal.flush().unwrap();
            let mut ignored = [0];
            terminal.read_exact(&mut ignored).unwrap();
        }
        #[cfg(unix)]
        "terminal-marker-wait" => {
            let mut terminal = wait_for_foreground_terminal();
            terminal.write_all(b"TERMINAL DROP READY\n").unwrap();
            terminal.flush().unwrap();
            fs::write(env::var_os(REEXEC_MARKER).unwrap(), b"ready").unwrap();
            let mut ignored = [0];
            terminal.read_exact(&mut ignored).unwrap();
        ...
        #[cfg(unix)]
        "terminal-supervisor-success" => terminal_supervisor("success"),
        #[cfg(unix)]
        "terminal-supervisor-nonzero" => terminal_supervisor("nonzero"),
        #[cfg(unix)]
        "terminal-supervisor-fast" => terminal_supervisor("fast"),
        #[cfg(unix)]
        "terminal-supervisor-timeout" => terminal_supervisor("timeout"),
        #[cfg(unix)]
        "terminal-supervisor-drop" => terminal_supervisor("drop"),
        #[cfg(unix)]
        "terminal-supervisor-cancel" => terminal_supervisor("cancel"),
...
#[cfg(unix)]
fn wait_for_foreground_terminal() -> File {
    let terminal = File::options().read(true).write(true).open("/dev/tty").unwrap();
    // SAFETY: getpgrp has no preconditions and cannot fail.
    let process_group = unsafe { libc::getpgrp() };
    let deadline = Instant::now() + Duration::from_secs(2);
    loop {
        // SAFETY: `terminal` is a live controlling-terminal descriptor.
        let foreground = unsafe { libc::tcgetpgrp(terminal.as_raw_fd()) };
        assert_ne!(foreground, -1, "failed to inspect terminal foreground ownership");
        if foreground == process_group {
            return terminal;
        }
        assert!(Instant::now() < deadline, "fixture never received terminal foreground ownership");
        thread::yield_now();
    }
}
...
fn assert_terminal_restored() {
    let mut terminal = wait_for_foreground_terminal();
    terminal.write_all(b"TERMINAL RESTORED\n").unwrap();
    terminal.flush().unwrap();
}

#[cfg(unix)]
fn terminal_supervisor(case: &str) {
    match case {
        "success" | "nonzero" => {
            let expected = if case == "success" { 0 } else { 23 };
            let mut command = reexec("terminal-prompt");
            command.env(REEXEC_BYTES, expected.to_string());
            let runtime =
                tokio::runtime::Builder::new_current_thread().enable_time().build().unwrap();
            let output = runtime.block_on(output(command, TEST_TIMEOUT)).unwrap();
            assert_eq!(
                output.status().code(),
                Some(expected),
        ...
        "timeout" => {
            let runtime =
                tokio::runtime::Builder::new_current_thread().enable_time().build().unwrap();
            let error = runtime
                .block_on(output(reexec("terminal-wait"), Duration::from_millis(100)))
                .unwrap_err();
        ...
        "fast" => {
            let runtime =
                tokio::runtime::Builder::new_current_thread().enable_time().build().unwrap();
            for _ in 0..32 {
                let output = runtime.block_on(output(reexec("nonzero"), TEST_TIMEOUT)).unwrap();
                assert_eq!(output.status().code(), Some(23));
            }
        }
        "drop" => {
            let directory = tempfile::tempdir().unwrap();
            let marker = directory.path().join("terminal-owned");
            let mut command = reexec("terminal-marker-wait");
            command
                .env(REEXEC_MARKER, &marker)
                .stdin(Stdio::null())
                .stdout(Stdio::null())
                .stderr(Stdio::null());
            let child = spawn_owned_fixture(&mut command);
            wait_for_marker_blocking(&marker);
            drop(child);
        }
        "cancel" => {
            let directory = tempfile::tempdir().unwrap();
            ...
            let runtime =
                tokio::runtime::Builder::new_current_thread().enable_time().build().unwrap();
            let task = runtime.spawn(output(command, Duration::from_secs(30)));
            runtime.block_on(wait_for_marker(&marker));
            task.abort();
            assert!(runtime.block_on(task).unwrap_err().is_cancelled());
        }
        _ => unreachable!("the fixture supplies one known terminal case"),
    }
...
#[cfg(unix)]
#[test]
fn controlling_terminal_prompts_complete_and_restore_the_caller() {
    run_terminal_session("terminal-supervisor-success", true, b"TERMINAL PROMPT");
}

#[cfg(unix)]
#[test]
fn nonzero_completion_restores_the_controlling_terminal() {
    run_terminal_session("terminal-supervisor-nonzero", true, b"TERMINAL PROMPT");
}

#[cfg(unix)]
#[test]
fn immediate_completion_preserves_status_and_restores_the_terminal() {
    run_terminal_session("terminal-supervisor-fast", false, b"TERMINAL RESTORED");
}

#[cfg(unix)]
#[test]
fn timeout_cleanup_restores_the_controlling_terminal() {
    run_terminal_session("terminal-supervisor-timeout", false, b"TERMINAL PROMPT");
}

#[cfg(unix)]
#[test]
fn owned_child_drop_restores_the_controlling_terminal() {
    run_terminal_session("terminal-supervisor-drop", false, b"TERMINAL DROP READY");
}

#[cfg(unix)]
#[test]
fn future_cancellation_restores_the_controlling_terminal() {
    run_terminal_session("terminal-supervisor-cancel", false, b"TERMINAL DROP READY");
}
...
fn run_terminal_session(mode: &str, reply: bool, ready: &[u8]) {
    ...
    // SAFETY: only async-signal-safe session and terminal ioctls run between
    // fork and exec. Standard input has already been installed from `slave`,
    // which remains live in the child through this closure.
    unsafe {
        command.pre_exec(|| {
            if libc::setsid() == -1 {
                return Err(io::Error::last_os_error());
            }
            // `libc` uses different request types on macOS and the same type
            // on Linux.
    ...
    let mut child = KillOnDrop(Some(command.spawn().unwrap()));
    drop(slave);

    let mut writer = master.try_clone().unwrap();
    let (sender, receiver) = mpsc::channel();
    let reader = thread::spawn(move || read_terminal(master, sender));
    let mut transcript = Vec::new();
    receive_terminal_text(&receiver, &mut transcript, ready);
    if reply {
        writer.write_all(b"ok\n").unwrap();
        writer.flush().unwrap();
    }
    receive_terminal_text(&receiver, &mut transcript, b"TERMINAL RESTORED");

    let status = child.0.as_mut().unwrap().wait().unwrap();
    assert!(status.success(), "terminal fixture failed with {status}: {transcript:?}");
    child.0 = None;
    drop(writer);
    reader.join().unwrap();
}
...
#[cfg(unix)]
fn open_pseudoterminal() -> (File, File) {
    let mut master = -1;
    let mut slave = -1;
    // SAFETY: both integers are exclusive writable outputs. Null termios,
    // window-size, and name pointers request the platform defaults.
    let result = unsafe {
        libc::openpty(
            &mut master,
            &mut slave,
    ...
    assert_eq!(result, 0, "failed to open a pseudoterminal: {}", io::Error::last_os_error());
    // SAFETY: successful openpty returned two new uniquely owned descriptors.
    unsafe { (File::from_raw_fd(master), File::from_raw_fd(slave)) }
}

#[cfg(unix)]
fn read_terminal(mut master: File, sender: mpsc::Sender<io::Result<Vec<u8>>>) {
    ...
    loop {
        match master.read(&mut buffer) {
            ...
            // Linux reports EIO when the final pseudoterminal slave closes.
...
#[cfg(unix)]
fn receive_terminal_text(
    receiver: &mpsc::Receiver<io::Result<Vec<u8>>>,
    transcript: &mut Vec<u8>,
    expected: &[u8],
) {
    let deadline = Instant::now() + Duration::from_secs(3);
    while !transcript.windows(expected.len()).any(|window| window == expected) {
        let remaining = deadline.saturating_duration_since(Instant::now());
        assert!(!remaining.is_zero(), "terminal fixture did not emit {expected:?}: {transcript:?}");
        let bytes = receiver
            .recv_timeout(remaining)
            .unwrap_or_else(|error| panic!("terminal fixture stalled before {expected:?}: {error}"))
            .unwrap();
...
fn unwind_drop_kills_the_owned_process_group() {
    ...
    command
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    let owned = spawn_owned_fixture(&mut command);
...
#[cfg(unix)]
fn spawn_owned_fixture(command: &mut Command) -> OwnedChild {
    let cancelled = AtomicBool::new(false);
    let deadline = Deadline::execution(Instant::now() + TEST_TIMEOUT, DeadlineFault::None);
    match spawn_owned(command, deadline, &cancelled) {
        Ok(child) => child,
        Err(StartError::BeforeSpawn(error)) => panic!("failed to spawn fixture: {error}"),
        Err(StartError::BeforeSpawnCommand(error)) => {
            panic!("failed to prepare fixture startup: {error}")
        }
        Err(StartError::AfterSpawn { child, error }) => {
            drop(child);
            panic!("failed after spawning fixture: {error}")
        }
    }
}
