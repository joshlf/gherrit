...
pub(super) const INITIAL_GRAPHQL_BATCH_LEN: usize = 64;
...
impl BatchPlan {
    pub fn new(item_count: usize, max_batch_len: usize) -> Self {
        assert!(max_batch_len != 0, "batch length must be nonzero");
        Self { item_count, cursor: 0, batch_len: max_batch_len }
    }

    pub fn current(&self) -> Option<Range<usize>> {
        (self.cursor < self.item_count)
            .then(|| self.cursor..self.item_count.min(self.cursor + self.batch_len))
    }
    ...
    pub fn reject(&mut self) -> Result<Backoff, ItemTooLarge> {
        ...
        let retry = attempted / 2;
        self.batch_len = retry;
        Ok(Backoff { attempted, retry })
    }
}
...
mod tests {
    ...
    fn floors_odd_backoff_lengths() {
        let mut plan = BatchPlan::new(3, 3);
