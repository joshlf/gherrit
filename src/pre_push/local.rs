...
///
/// Constructing a `ChangeId` is validation. Code which has one can therefore
...
impl LocalChange {
    fn from_git(commit: gix::Commit<'_>, title: String, trailers: &[u8]) -> Result<Self> {
        ...
        let id = ChangeId::from_trailer(commit.id, id)?;
...
impl LocalStack {
    ...
    pub(super) fn collect(
        ...
        let default_ref = repo.rev_parse_single(default_branch.full_ref_name().as_str())?;
        ...
        let changes = commits
            .into_iter()
            .zip(trailers)
            .map(|((commit, title), trailers)| LocalChange::from_git(commit, title, &trailers))
            .collect::<Result<Vec<_>>>()?;

        Self::new(default_ref.detach(), default_branch.name(), changes)
    }
...
mod tests {
    ...
    fn change(id: &str, head: u8, first_parent: u8) -> LocalChange {
        LocalChange {
            id: ChangeId::from_trailer(object_id(head), id.as_bytes()).unwrap(),
            head: object_id(head),
    ...
    #[test]
    fn change_ids_accept_exactly_nonempty_ascii_alphanumeric_trailers() {
        for id in [b"A".as_slice(), b"G123", b"abcDEF012"] {
            assert_eq!(
                ChangeId::from_trailer(object_id(1), id).unwrap().as_str(),
                str::from_utf8(id).unwrap()
            );
        }

        for id in [b"".as_slice(), b"with-dash", b"with space", "snowman-☃".as_bytes()] {
            assert!(ChangeId::from_trailer(object_id(1), id).is_err(), "id={id:?}");
        }
    }
