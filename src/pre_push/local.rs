...
mod tests {
    ...
    fn collected_titles_enforce_the_unicode_scalar_limit() {
        ...
        let collect = || {
            ...
            LocalStack::collect(&repository, "feature", head, &supplied_default)
        };
    ...
    fn collection_uses_the_captured_head_after_the_worktree_moves() {
        ...
        let stack =
            LocalStack::collect(&repository, "captured-stack", captured_head, &supplied_default)
                .unwrap();
    ...
    fn empty_stack_does_not_require_publishable_history() {
        ...
        let head = repository.head_snapshot().unwrap().target().unwrap();
        let stack = LocalStack::collect(&repository, "main", head, &supplied_default).unwrap();
