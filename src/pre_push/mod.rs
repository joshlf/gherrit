    ...
    O: MutationOperation,
{
    ...
    for batch in batches {
        ...
        let response =
            octocrab::map_github_error(response).await.wrap_err(INDETERMINATE_GRAPHQL_MUTATION)?;
        let response =
            octocrab.body_to_string(response).await.wrap_err(INDETERMINATE_GRAPHQL_MUTATION)?;
        let response =
            UniqueJson::decode(response.as_bytes()).wrap_err(INDETERMINATE_GRAPHQL_MUTATION)?;
