use std::{collections::HashMap, env, ffi::OsStr, process::Stdio, time::Duration};
...
/// security boundary. Binding it to Git's remote-name argument prevents an
/// inherited marker from disabling GHerrit for an unrelated nested push.
...
fn push_to_origin(
    ...
    for chunk in push_batches(commits.as_slice()) {
        ...
        let status = destination
            .push(options, refspecs)
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .status()
            .wrap_err("Failed to run `git push`")?;
        if !status.success() {
            // Git can normalize a URL or local path before printing it. No
            // destination-bearing command output crosses this boundary.
            bail!(
                "`git push` failed for GHerrit remote '{}'. Its push destination might be ahead or have changed.",
...
async fn sync_prs(
            ...
            let pr_state = match resolution {
                ...
                PrResolution::ToCreate(create) => {
                    ...
                    log::info!(
                        ...
                        destination.pr_url(created.number).blue().underline()
                    );
                    PrState {
    ...
    let updates: Vec<(u64, PrUpdate)> = commit_pr_states
        ...
        .filter_map(|(entry, pr_state)| {
            ...
            let pr_url = destination.pr_url(pr_state.number).blue().underline().to_string();
...
struct CreatedPullRequestIdentitySet {
    node_by_number: HashMap<u64, String>,
    number_by_node: HashMap<String, u64>,
}
...
async fn batch_update_prs(octocrab: &Octocrab, updates: Vec<(u64, PrUpdate)>) -> Result<()> {
    let updates = updates
        .into_iter()
        .map(|(number, update)| {
...
mod tests {
    ...
    fn internal_push_marker_must_match_complete_hook_arguments() {
        ...
        for (remote_name, remote_location, marker) in [
            (Some("origin"), Some("private-destination"), Some(marker)),
            (Some("gherrit-publication-2"), Some(""), Some(marker)),
            (Some(""), Some("private-destination"), Some(marker)),
            (Some("gherrit-publication-2"), None, Some(marker)),
            (None, Some("private-destination"), Some(marker)),
            (Some("gherrit-publication-2"), Some("private-destination"), None),
        ] {
