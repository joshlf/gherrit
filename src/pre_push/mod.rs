...
use publication::{
    Checkpoint, CheckpointTarget, PublicationAction, PushTarget, confirmed_checkpoint,
    parse_version, plan_publication, plan_push, push_batches,
};
use reconcile::{
...
fn push_to_origin(repo: &util::Repo, commits: Vec<Commit>) -> Result<Vec<PublishedCommit>> {
    ...
    let actions = commits
        .iter()
        .zip(local_checkpoints.iter().copied())
    ...
    let pending = actions
        ...
        .filter_map(|action| match action {
            ...
            PublicationAction::NoPush(_) => None,
        })
    ...
    let confirmed_publications = observe_publications(repo, &gherrit_ids)
        .wrap_err("Failed to confirm remote publication state before PR projection")?;
    if confirmed_publications.len() != commits.len() {
        bail!(
            "Git returned {} publication confirmations for {} commits",
            confirmed_publications.len(),
            commits.len()
        );
    }
    let checkpoints = actions
        .iter()
        .zip(confirmed_publications)
        .map(|(action, remote)| confirmed_checkpoint(action.required_checkpoint(), remote))
        .collect::<Result<Vec<_>>>()?;
    commits.iter().zip(&local_checkpoints).zip(&checkpoints).try_for_each(
        |((commit, local), checkpoint)| {
            if *local != Some(*checkpoint) {
                persist_local_checkpoint(
                    repo,
                    CheckpointTarget { gherrit_id: &commit.gherrit_id, checkpoint: *checkpoint },
                )?;
            }
    ...
    let versions = checkpoints.iter().map(|checkpoint| checkpoint.version).collect::<Vec<_>>();
    drop(pending);
    drop(actions);
    drop(gherrit_ids);
    Ok(commits
...
fn persist_local_checkpoint(repo: &util::Repo, target: CheckpointTarget<'_>) -> Result<()> {
    ...
    if result.is_err()
        && repo
            ...
            .is_some_and(|object_id| object_id.detach() == target.checkpoint.object_id)
    {
        return Ok(());
    }
