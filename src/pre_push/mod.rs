...
pub async fn run(repo: &util::Repo, github_endpoint: &GithubEndpoint) -> Result<()> {
    ...
    let branch_ref = repo.rev_parse_single(format!("refs/heads/{branch_name}").as_str())?.detach();
    if branch_ref != head {
        bail!("The checked-out branch changed while GHerrit captured publication intent");
    }
...
async fn sync_prs(
    ...
    let new_prs = if !creations.is_empty() {
        ...
        let created = batch_create_prs(octocrab, &repository.node_id, &prs, creations).await?;
    ...
    let repo_fetch_url = destination.repo_fetch_url();
