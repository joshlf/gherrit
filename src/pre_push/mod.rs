...
pub async fn run(repo: &util::Repo, github_endpoint: &GithubEndpoint) -> Result<()> {
    ...
    // The two reads are deliberately not treated as one snapshot. An atomic
    // concurrent publisher between them can make an old head disagree with
    // newer tags; normalization then fails safely and a retry observes both
    // again. Branch and absent-tag leases protect every later write.
