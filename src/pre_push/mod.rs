...
async fn sync_prs(
        ...
        .map(|(index, entry)| {
            ...
            if let Some(pr) = prs.iter().find(|pr| pr.head_branch == c.id().as_str()) {
                ...
                PrResolution::ToCreate(BatchCreate {
                    ...
                    head_oid: c.head(),
                    // A nonroot's owned base was just published at this exact
                    // object. The root's default base was read only from the
                    // local remote-tracking branch at this boundary, so GitHub
                    // may legitimately report a newer default tip.
                    expected_base_oid: (index != 0).then(|| c.first_parent()),
                })
            }
        })
        .collect();
    ...
    let new_prs = if !creations.is_empty() {
        ...
        let created = batch_create_prs(octocrab, &repo_id, &prs, creations).await?;
...
struct BatchCreate {
    ...
    expected_base_oid: Option<gix::ObjectId>,
}
...
impl CreatedPullRequestIdentitySet {
    fn from_observed(pull_requests: &[PrState]) -> Result<Self> {
        let mut identities = Self::default();
        for pull_request in pull_requests {
            identities.insert_identity(
                pull_request.number,
                pull_request.node_id.clone(),
                &format!("observed pull request for '{}'", pull_request.head_branch),
            )?;
        }
        Ok(identities)
    }
    ...
    fn insert_identity(
        ...
        if self.node_by_number.contains_key(&number) {
            bail!("{source} reuses pull request number {number}");
        }
        if self.number_by_node.contains_key(&node_id) {
            bail!("{source} reuses pull request node ID '{node_id}'");
        }

        self.node_by_number.insert(number, node_id.clone());
        self.number_by_node.insert(node_id, number);
        Ok(())
    }
    ...
    fn insert(&mut self, receipts: &[CreatedPullRequest]) -> Result<()> {
        for receipt in receipts {
            self.insert_identity(
                ...
                &format!("createPullRequest receipt for '{}'", receipt.head_branch),
            )?;
        }
        Ok(())
    }
...
async fn batch_create_prs(
    octocrab: &Octocrab,
    repo_id: &str,
    observed_pull_requests: &[PrState],
    ...
    let creations = creations.into_iter().map(|create| {
        CreatePullRequest::new(
            ...
            create.body,
            create.head_oid,
            create.expected_base_oid,
        )
    });
    let mut identities = CreatedPullRequestIdentitySet::from_observed(observed_pull_requests)?;
    ...
    O: MutationOperation,
{
    ...
    for batch in batches {
        ...
        let response = octocrab
            ._post("/graphql", Some(&batch.request))
            .await
            .wrap_err(INDETERMINATE_GRAPHQL_MUTATION)?;
        let status = response.status();
        if !status.is_success() {
            bail!("{INDETERMINATE_GRAPHQL_MUTATION}: GitHub returned HTTP {status}");
        }
        let response = Limited::new(response.into_body(), MAX_GRAPHQL_MUTATION_RESPONSE_BYTES)
            .collect()
            .await
            .map_err(|error| {
                if error.is::<LengthLimitError>() {
                    eyre!(
                        "GraphQL mutation response exceeds the \
                         {MAX_GRAPHQL_MUTATION_RESPONSE_BYTES}-byte body limit"
                    )
                } else {
                    eyre!("Failed to read GraphQL mutation response body: {error}")
                }
            })
            .wrap_err(INDETERMINATE_GRAPHQL_MUTATION)?
            .to_bytes();
        let response = UniqueJson::decode(&response).wrap_err(INDETERMINATE_GRAPHQL_MUTATION)?;
        let receipts = decode_mutation_batch_response(operations, response.into_value())
            .wrap_err(INDETERMINATE_GRAPHQL_MUTATION)?;
...
mod tests {
    ...
    fn observed_pull_request(number: u64, node_id: &str) -> PrState {
        FindPullRequest::new("owner".to_owned(), "repository".to_owned(), "Gobserved".to_owned())
            .decode(json!({
                "open": {
                    "nodes": [{
                        "number": number,
    ...
    fn batch_creation(index: usize) -> BatchCreate {
        BatchCreate {
            ...
            expected_base_oid: Some(test_object_id(b'2')),
        }
    }
    ...
    async fn assert_create_receipt_cannot_reuse_observed_identity(
        observed: PrState,
        expected: &str,
    ) {
        let listener = TcpListener::bind("127.0.0.1:0").await.expect("bind test server");
        ...
        let server = tokio::spawn(async move {
            ...
            tokio::select! {
                accepted = listener.accept() => {
                    accepted.expect("accept unexpected repeated mutation request");
                    panic!("an invalid create receipt was followed by another request");
                }
                _ = finished_rx => {}
            }
            ids
        ...
        let result = tokio::time::timeout(
            ADAPTER_TIMEOUT,
            batch_create_prs(&octocrab, "REPO_NODE_ID", &[observed], [batch_creation(0)]),
        )
        .await
        .expect("mutation attempt completed");
        let _ = finished_tx.send(());
        let ids = tokio::time::timeout(ADAPTER_TIMEOUT, server)
            .await
        ...
        assert_eq!(ids, [0], "the create mutation must be transmitted exactly once");
    }

    #[tokio::test]
    async fn create_receipts_cannot_reuse_observed_pull_request_identities() {
        assert_create_receipt_cannot_reuse_observed_identity(
            observed_pull_request(1, "PR_observed"),
            "reuses pull request number 1",
        )
        .await;
        assert_create_receipt_cannot_reuse_observed_identity(
            observed_pull_request(99, "PR_1"),
            "reuses pull request node ID 'PR_1'",
        )
        .await;
    }
    ...
    async fn assert_cross_batch_create_receipt_collision(collision: CreateReceiptCollision) {
        ...
        let creations = (0..OPERATION_COUNT).map(batch_creation);
        ...
        let expected = match collision {
            CreateReceiptCollision::Number => "reuses pull request number 1",
            CreateReceiptCollision::NodeId => "reuses pull request node ID 'PR_1'",
        };
        assert!(format!("{error:?}").contains(expected), "error={error:?}");
        assert_eq!(
            observation.requests,
    ...
    #[tokio::test]
    async fn oversized_successful_mutation_response_is_indeterminate_and_not_replayed() {
        let listener = TcpListener::bind("127.0.0.1:0").await.expect("bind test server");
        let octocrab = test_octocrab(&listener);
        let (finished_tx, finished_rx) = oneshot::channel();
        let server = tokio::spawn(async move {
            ...
            let body = vec![b' '; MAX_GRAPHQL_MUTATION_RESPONSE_BYTES + 1];
            write_http_response(&mut stream, "200 OK", &body, body.len()).await;

            tokio::select! {
                accepted = listener.accept() => {
                    accepted.expect("accept unexpected repeated mutation request");
                    panic!("an oversized mutation response was followed by another request");
                }
                _ = finished_rx => {}
            }
            ids
        ...
        let result = tokio::time::timeout(
            ADAPTER_TIMEOUT,
            run_graphql_mutations(&octocrab, [TestMutation::new(0)]),
        )
        .await
        .expect("mutation attempt completed");
        let _ = finished_tx.send(());
        ...
        let error = result.expect_err("an oversized response must end the attempt");
        assert!(error.to_string().contains("indeterminate"), "error={error:?}");
        assert!(format!("{error:?}").contains("4194304-byte body limit"), "error={error:?}");
        assert_eq!(ids, [0], "the mutation must be transmitted exactly once");
    }
