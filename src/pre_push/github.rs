use std::{collections::HashSet, fmt, num::NonZeroU32, ops::Range};
...
/// A nonempty opaque GraphQL node ID for one pull request.
...
impl fmt::Display for PullRequestNodeId {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.0.fmt(formatter)
    }
}
...
pub(super) struct PullRequest {
    ...
    pub(super) node_id: PullRequestNodeId,
...
impl QueryOperation for FindPullRequest {
    ...
    fn decode(&self, response: Value) -> Result<Self::Output> {
        ...
        Ok(Some(PullRequest {
            number: PullRequestNumber::new(node.number)?,
            node_id: PullRequestNodeId::new(node.id)?,
            title: node.title,
            body: node.body,
...
pub(super) struct CreatePullRequest {
    ...
    body: String,
    head_oid: ObjectId,
    expected_base_oid: Option<ObjectId>,
...
impl MutationOperation for CreatePullRequest {
    ...
    fn decode_receipt(&self, response: Value) -> Result<Self::Output> {
        ...
        let base_oid = parse_oid("base", &created.base_ref_oid)?;
        if self.expected_base_oid.is_some_and(|expected| base_oid != expected) {
            bail!("createPullRequest returned a different base object ID");
        }
        let number = PullRequestNumber::new(created.number)
            .wrap_err("createPullRequest returned an invalid pull request number")?;
        let node_id = PullRequestNodeId::new(created.id)
            .wrap_err("createPullRequest returned an invalid pull request node ID")?;

        Ok(CreatedPullRequest { head_branch: self.head_branch.clone(), number, node_id })
    }
    ...
    fn validate_receipts(receipts: &[Self::Output]) -> Result<()> {
        ...
        for (index, receipt) in receipts.iter().enumerate() {
            ...
            if !node_ids.insert(&receipt.node_id) {
                bail!(
...
impl MutationOperation for UpdatePullRequest {
    ...
    fn document(&self) -> String {
        let fields = std::iter::once(("pullRequestId", self.node_id.as_str()))
            .chain(self.base_branch.as_ref().map(|value| ("baseRefName", value.as_str())))
            .chain(self.title.as_ref().map(|value| ("title", value.as_str())))
            .chain(self.body.as_ref().map(|value| ("body", value.as_str())))
    ...
    fn decode_receipt(&self, response: Value) -> Result<Self::Output> {
        ...
        let updated_number = PullRequestNumber::new(updated.number)
            .wrap_err("updatePullRequest returned an invalid pull request number")?;
        let updated_node_id = PullRequestNodeId::new(updated.id)
            .wrap_err("updatePullRequest returned an invalid pull request node ID")?;
        if updated_number != self.number || updated_node_id != self.node_id {
            bail!(
...
mod tests {
    ...
    fn pull_request_number(value: u64) -> PullRequestNumber {
        PullRequestNumber::new(value).unwrap()
    }

    fn pull_request_node_id(value: &str) -> PullRequestNodeId {
        PullRequestNodeId::new(value.to_string()).unwrap()
    }
    ...
    fn update_document_omits_unchanged_fields() {
        ...
        assert_eq!(
            update.document(),
            r#"updatePullRequest(input: { pullRequestId: "PR_node", title: "new \"title\"", clientMutationId: "gherrit:update:PR_node" }) { clientMutationId, pullRequest { number, id, state } }"#
        );
    }
    ...
    fn batch_document_aliases_each_operation_exactly() {
        ...
        assert_eq!(
            ...
            r#"mutation { op0: updatePullRequest(input: { pullRequestId: "PR_1", title: "one", clientMutationId: "gherrit:update:PR_1" }) { clientMutationId, pullRequest { number, id, state } }op1: updatePullRequest(input: { pullRequestId: "PR_2", title: "two", clientMutationId: "gherrit:update:PR_2" }) { clientMutationId, pullRequest { number, id, state } } }"#
        );
    }
    ...
    fn decodes_pull_request_queries_to_owned_state() {
        ...
        assert_eq!(
            query
                ...
                .unwrap(),
            Some(PullRequest {
                number: pull_request_number(42),
                node_id: pull_request_node_id("PR_42"),
    ...
    #[test]
    fn pull_request_identity_types_represent_exactly_the_accepted_domain() {
        for value in [1, i32::MAX as u64] {
            let number = PullRequestNumber::new(value).unwrap();
            assert_eq!(number.get(), value);
            assert_eq!(number.to_string(), value.to_string());
        }
        for value in [0, i32::MAX as u64 + 1, u64::MAX] {
            let error = PullRequestNumber::new(value).unwrap_err();
            assert_eq!(
                error.to_string(),
                format!("GitHub reported invalid pull request number {value}")
            );
        }

        let node_id = PullRequestNodeId::new("PR_opaque".to_string()).unwrap();
        assert_eq!(node_id.as_str(), "PR_opaque");
        assert_eq!(node_id.to_string(), "PR_opaque");
        assert_eq!(
            PullRequestNodeId::new(String::new()).unwrap_err().to_string(),
            "GitHub reported an empty pull request node ID"
        );
    }

    #[test]
    fn selected_pull_request_identity_must_be_valid() {
        let query =
            FindPullRequest::new("owner".to_string(), "repo".to_string(), "G123".to_string());
        let node = |number, node_id: &str, state| {
            let mut node = pull_request_node(number, state, false);
        ...
        let cases = [
            (
                lookup_response(
                    connection(vec![node(0, "PR_0", "OPEN")], false),
                    empty_connection(),
                ),
            ...
            (
                lookup_response(
                    empty_connection(),
                    connection(vec![node(i32::MAX as u64 + 1, "PR_too_large", "MERGED")], false),
                ),
                "invalid pull request number 2147483648",
            ),
            (
                lookup_response(connection(vec![node(42, "", "OPEN")], false), empty_connection()),
                "empty pull request node ID",
            ),
        ];

        cases.into_iter().for_each(|(response, expected)| {
            let error = query.decode(response).unwrap_err();
    ...
    fn a_unique_open_pull_request_wins_over_history() {
        ...
        assert_eq!(selected.number, pull_request_number(42));
    ...
    fn update_receipt(node_id: &str) -> Value {
        ...
        json!({
            ...
            "pullRequest": { "number": number, "id": node_id, "state": "OPEN" },
        })
    }
    ...
    fn an_update_must_change_at_least_one_field() {
        let error = UpdatePullRequest::new(
            pull_request_number(1),
            pull_request_node_id("PR_1"),
            None,
            None,
    ...
    fn update_receipts_must_match_the_request_and_target() {
        ...
        let mut zero_number = update_receipt("PR_1");
        zero_number["pullRequest"]["number"] = json!(0);
        let mut oversized_number = update_receipt("PR_1");
        oversized_number["pullRequest"]["number"] = json!(i32::MAX as u64 + 1);
        let mut empty_node = update_receipt("PR_1");
        empty_node["pullRequest"]["id"] = json!("");
        let mut closed = update_receipt("PR_1");
        closed["pullRequest"]["state"] = json!("CLOSED");
        let mut merged = update_receipt("PR_1");
        merged["pullRequest"]["state"] = json!("MERGED");
        let mut unknown_state = update_receipt("PR_1");
        unknown_state["pullRequest"]["state"] = json!("UNKNOWN");
        let mut missing_state = update_receipt("PR_1");
        missing_state["pullRequest"].as_object_mut().unwrap().remove("state");
        let cases = [
            ...
            (zero_number, "invalid pull request number 0"),
            (oversized_number, "invalid pull request number 2147483648"),
            (empty_node, "empty pull request node ID"),
            (closed, "is not OPEN"),
            (merged, "is not OPEN"),
            (unknown_state, "unknown variant `UNKNOWN`"),
            (missing_state, "missing field `state`"),
            (
                json!({
                    "pullRequest": { "number": 1, "id": "PR_1", "state": "OPEN" }
                }),
    ...
    fn create_receipt_numbers_must_fit_graphql_int() {
        ...
        assert_eq!(
            ...
            [CreatedPullRequest {
                head_branch: "G123".to_string(),
                number: pull_request_number(maximum),
                node_id: pull_request_node_id("PR_max"),
            }]
        );
    ...
    #[test]
    fn create_receipt_without_authoritative_base_still_validates_its_shape() {
        let mut operation = create();
        operation.expected_base_oid = None;
        let mut receipt = create_receipt();
        receipt["pullRequest"]["baseRefOid"] = json!(object_id(b'3').to_string());
        decode_mutation_batch_response(
            std::slice::from_ref(&operation),
            json!({ "data": { "op0": receipt } }),
        )
        ...
        let mut malformed = create_receipt();
        malformed["pullRequest"]["baseRefOid"] = json!("invalid");
        let error = decode_mutation_batch_response(
            std::slice::from_ref(&operation),
            json!({ "data": { "op0": malformed } }),
        )
        .unwrap_err();
        assert!(format!("{error:?}").contains("invalid base object ID"));
    }
}
