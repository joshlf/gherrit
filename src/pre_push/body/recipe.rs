...
/// raw public branch.
///
/// The repository URL is a validated GitHub-relative path. Web links use it
/// directly, while download commands use the canonical fetch URL derived from
/// the same repository coordinates. Neither projection depends on a local
/// remote name or retains the potentially private destination literal.
///
...
impl BodyLinkContext {
    fn from_destination(
        ...
        Ok(Self {
            repository_url: destination.repo_url_relative(),
            repository_fetch_url: destination.repo_fetch_url(),
            public_branch,
        })
    }
    ...
    fn for_test(repository_url: &str, public_branch: Option<&str>) -> Result<Self> {
        ...
        Ok(Self {
            ...
            repository_fetch_url: format!("https://github.com{repository_url}.git"),
            public_branch,
        })
    }
}
...
fn render_preferred_body(
    ...
    render_body(context, id, current_index, commit_body, history, navigation, HistoryLayout::Full)
...
fn write_body(
    ...
    output.write_str("\n\n---\n\n")?;
    write_navigation(output, context.public_branch.as_deref(), current_index, navigation)?;
    write_history(output, &context.repository_url, id, history, history_layout)?;
    write_download(output, &context.repository_fetch_url, id)?;
    output.write_str("\n\n")?;
...
fn write_download(output: &mut impl fmt::Write, repository: &str, id: &GherritPrId) -> fmt::Result {
    output.write_str("\n<details>\n<summary><strong>⬇️ Download this PR</strong></summary>\n\n")?;
    output.write_str("######\n\n")?;
    let id = id.as_str();
    writeln!(
        output,
        "**Branch**\n```bash\ngit fetch {repository} refs/heads/{id} && git checkout -b pr-{id} FETCH_HEAD\n```\n"
    )?;
    writeln!(
        output,
        "**Checkout**\n```bash\ngit fetch {repository} refs/heads/{id} && git checkout FETCH_HEAD\n```\n"
    )?;
    writeln!(
        output,
        "**Cherry Pick**\n```bash\ngit fetch {repository} refs/heads/{id} && git cherry-pick FETCH_HEAD\n```\n"
    )?;
    writeln!(output, "**Pull**\n```bash\ngit pull {repository} refs/heads/{id}\n```\n")?;
    output.write_str("</details>")
}
...
mod tests {
    ...
    fn render_single(
        ...
        let context = link_context("/octo/widgets", None);
        render_body(
            &context,
            history.id(),
            0,
    ...
    #[test]
    fn download_commands_use_the_push_destination_instead_of_the_local_remote() {
        ...
        let recipes =
            StackBodyRecipes::from_parts(context, fixture.changes, fixture.histories).unwrap();
        let body = final_single(&recipes, number(7));
        let canonical_fetch_url = "https://github.com/push-owner/push-repository.git";

        assert_eq!(body.body().as_str().matches(canonical_fetch_url).count(), 4);
        assert!(!body.body().as_str().contains("git fetch origin"));
        assert!(!body.body().as_str().contains("git pull origin"));
        assert!(!body.body().as_str().contains("git@github.com"));
    }
    ...
    fn provisional_body_falls_back_to_sparse_only_when_needed() {
        ...
        let context = link_context("/octo/widgets", None);
        let render = |layout| {
            render_body(&context, history.id(), 0, "", history, Navigation::Omitted, layout)
                .unwrap()
                .as_str()
                .len()
        };
