    ...
    pub(in crate::pre_push) fn new(
        ...
        for (index, input) in inputs.iter().enumerate() {
            let full = render_body(
                ...
                inputs.iter().map(|entry| entry.change.id()),
                index,
                input.change.body(),
                &input.history,
...
fn render_body<'id>(
    ...
    ids: impl Clone
    + DoubleEndedIterator<Item = &'id GherritPrId>
    + ExactSizeIterator<Item = &'id GherritPrId>,
    current_index: usize,
    commit_body: &str,
    history: &ValidatedChangeHistory,
...
fn write_body<'id>(
    ...
    let id = ids.clone().nth(current_index).expect("the current recipe index is in the stack");
    let parent = current_index.checked_sub(1).and_then(|index| ids.clone().nth(index));
    let child = ids.clone().nth(current_index + 1);
    ...
    output.write_str("\n\n")?;
    output
        .write_str("*Stacked PRs enabled by [GHerrit](https://github.com/joshlf/gherrit).*\n\n")?;
    output.write_str(
        "<!-- WARNING: GHerrit relies on the following metadata to work properly. DO NOT EDIT OR REMOVE. -->\n",
    )?;
    write_metadata(output, id, parent, child)
}
...
fn write_metadata(
    ...
    output.write_str("<!-- gherrit-meta: {\"id\":\"")?;
    output.write_str(id.as_str())?;
    output.write_str("\",\"parent\":")?;
    write_optional_id(output, parent)?;
    output.write_str(",\"child\":")?;
    write_optional_id(output, child)?;
    output.write_str("} -->")
}

fn write_optional_id(output: &mut impl fmt::Write, value: Option<&GherritPrId>) -> fmt::Result {
    match value {
        Some(value) => {
            output.write_char('"')?;
...
mod tests {
    ...
    fn validated_history(
        ...
        if let Some((head, first_parent)) = published.last() {
            writeln!(heads, "{head}\trefs/heads/{}", change.id().as_str()).unwrap();
            writeln!(heads, "{first_parent}\trefs/heads/gherrit-bases/{}", change.id().as_str())
                .unwrap();
        }
    ...
    fn repeated_history_fixture(body: &str) -> AmendFixture {
        ...
        let revisions = vec![(a, base), (b, base), (a, base)];
    ...
    fn assert_metadata(body: &GeneratedBody, id: &str, parent: Option<&str>, child: Option<&str>) {
        assert_eq!(body.as_str().matches("<!-- gherrit-meta:").count(), 1);
        let marker = body.as_str().find("<!-- gherrit-meta:").unwrap();
        assert!(marker == 0 || body.as_str().as_bytes()[marker - 1] == b'\n');
        let metadata = &body.as_str()[marker..];
        assert!(metadata.starts_with("<!-- gherrit-meta: {\"id\":"));
        let id_field = metadata.find("\"id\":").unwrap();
        let parent_field = metadata.find("\"parent\":").unwrap();
        let child_field = metadata.find("\"child\":").unwrap();
        assert!(id_field < parent_field && parent_field < child_field);
        assert!(body.as_str().ends_with("} -->"));
        let parsed = parse_metadata(body.as_str()).unwrap().expect("one metadata record");
        assert_eq!(parsed.id().as_str(), id);
        assert_eq!(parsed.parent().map(GherritPrId::as_str), parent);
        assert_eq!(parsed.child().map(GherritPrId::as_str), child);
    }

    fn assert_ordered_metadata(bodies: &[RenderedBody], ids: &[&str]) {
        for body in bodies {
            let index = ids.iter().position(|id| *id == body.id().as_str()).unwrap();
            assert_metadata(
                body.body(),
                ids[index],
                index.checked_sub(1).map(|index| ids[index]),
                ids.get(index + 1).copied(),
            );
        }
    }

    fn assert_representative_metadata(recipes: &StackBodyRecipes, ids: &[&str]) {
        let bodies = recipes.final_bodies().representative_bodies().collect::<Vec<_>>();
        assert_eq!(bodies.len(), ids.len());
        for (index, (id, body)) in bodies.into_iter().enumerate() {
            assert_eq!(id.as_str(), ids[index]);
            assert_metadata(
                body,
                ids[index],
                index.checked_sub(1).map(|index| ids[index]),
    ...
    fn assert_all_recipe_metadata(recipes: &StackBodyRecipes, ids: &[&str]) {
        assert_ordered_metadata(recipes.provisional_bodies(), ids);
        assert_representative_metadata(recipes, ids);
    }
    ...
    fn complete_final_stack_snapshot_uses_existing_and_assigned_numbers() {
        ...
        let bodies = final_bodies
            ...
            .unwrap();
        assert_ordered_metadata(&bodies, &ids);
    ...
    fn generated_body_accepts_exactly_the_limit_and_rejects_the_next_byte() {
        ...
        assert_metadata(&exact, "Gexact", None, None);
    ...
    fn widest_full_layout_succeeds_at_the_exact_boundary() {
        ...
        assert_metadata(representative, "Gfullexact", None, None);
        assert_metadata(recipes.provisional_bodies()[0].body(), "Gfullexact", None, None);
        ...
        assert_metadata(bodies[0].body(), "Gfullexact", None, None);
    }
    ...
    fn widest_sparse_fallback_is_frozen_for_short_actual_numbers() {
        ...
        assert_all_recipe_metadata(&recipes, &["Gsparse"]);
        ...
        assert_metadata(&recipes.final_bodies.entries[0].representative, "Gsparse", None, None);
        let bodies = complete_missing(recipes, &[("Gsparse", 7)]).unwrap();
        assert!(!bodies[0].body().as_str().contains(full_only));
        assert_metadata(bodies[0].body(), "Gsparse", None, None);
    }
    ...
    fn layout_is_selected_and_frozen_per_entry() {
        ...
        assert_all_recipe_metadata(&recipes, &["Gfull", "Gsparse"]);
        assert_eq!(recipes.final_bodies.entries[0].layout, HistoryLayout::Full);
        ...
        assert_ordered_metadata(&bodies, &["Gfull", "Gsparse"]);
    }
    ...
    fn repeated_pending_sentinel_uses_index_at_the_exact_boundary() {
        ...
        let rendered = render_body(
            "/octo/widgets",
            None,
            [change_id(ids[0]), change_id(ids[1])].iter(),
            0,
            "",
            &histories[0],
            Navigation::Numbered(&[
        ...
        let recipes = missing_recipes("/octo/widgets", None, stack, histories).unwrap();
        assert_all_recipe_metadata(&recipes, &ids);
    ...
    fn amend_and_rebase_history_snapshot_keeps_literal_base_oids() {
        ...
        assert_all_recipe_metadata(&recipes, &["Ghistory"]);
        let representative = &recipes.final_bodies.entries[0].representative;
        assert_metadata(representative, "Ghistory", None, None);
    ...
    #[test]
    fn nonconsecutive_repeated_revision_snapshot_retains_all_positions() {
        ...
        let recipes =
            missing_recipes("/octo/widgets", None, fixture.stack, vec![fixture.history]).unwrap();
        assert_all_recipe_metadata(&recipes, &["Grepeat"]);
        let representative = &recipes.final_bodies.entries[0].representative;
        assert_metadata(representative, "Grepeat", None, None);
        assert!(representative.as_str().contains("**Latest Update:** v3"));
        let normalized = normalize_object_ids(
            representative.as_str().to_owned(),
            &[(shared_base, "<shared-literal-base>")],
        );
        insta::assert_snapshot!("bounded_nonconsecutive_a_b_a_history", normalized);
    }
}
