//! Pure publication-state normalization and old-format ref planning.
//!
...
use super::{
    local::{ChangeId, LocalChange, LocalStack},
    remote::{ActiveVersionTags, RemoteHeads},
    version::Version,
};
...
#[derive(Clone, Debug)]
enum PushTarget {
    First { id: ChangeId, desired_head: ObjectId },
    Advance { id: ChangeId, desired_head: ObjectId, expected_head: ObjectId, next_version: Version },
}

impl PushTarget {
    fn first(id: ChangeId, desired_head: ObjectId) -> Self {
...
/// Every old-format Git publication decision and ready push batch.
///
/// This transitional plan covers Git refs only. The change-owned-base
/// publisher replaces it with one cross-system plan whose construction also
/// validates GitHub state before exposing any executable batch.
#[derive(Debug)]
...
pub(super) fn plan_git_publication<'stack>(
    commits: &'stack LocalStack,
    heads: &RemoteHeads,
    versions: &ActiveVersionTags,
    ...
    let planned = commits
        .iter()
        .map(|change| {
            plan_change(change.id(), change.head(), heads, versions)
                .map(|publication| (change, publication))
        })
...
fn plan_change(
    ...
    if heads.owned_base(id).is_some() {
        bail!(
    ...
    let version_tags = versions.observed_tags_for(id)?;
    match normalize_remote_publication(id, heads.managed_head(id), version_tags)? {
        RemotePublication::Absent => {
            Ok(PlannedChange::Publish(PushTarget::first(id.clone(), desired_head)))
        }
        RemotePublication::Published(history) => {
            let latest_version = history.latest_version()?;
            let current_head = history.current_head();
...
/// A nonempty, ordered sequence of immutable remote version records.
///
/// Repeated object IDs remain separate entries because version numbers are
/// history records, not a set of distinct Git objects.
#[derive(Debug)]
...
impl PublishedVersions {
    fn current_head(&self) -> ObjectId {
        self.later.last().copied().unwrap_or(self.first)
    }

    fn latest_version(&self) -> Result<Version> {
        Version::from_history_index(self.later.len())
            .ok_or_else(|| eyre!("published version count does not fit u64"))
    }
...
fn normalize_remote_publication(
    ...
    let history = PublishedVersions { first, later: versions.collect::<Result<Vec<_>>>()? };
    if head != Some(history.current_head()) {
        bail!("Remote GHerrit change '{}' head does not match its latest version tag", id.as_str());
    }
    Ok(RemotePublication::Published(history))
}
...
mod tests {
    ...
    fn stack(changes: impl IntoIterator<Item = (ChangeId, ObjectId)>) -> LocalStack {
        LocalStack::for_test(object_id(0xff), changes)
    }

    fn remote(
        observed_ids: impl IntoIterator<Item = ChangeId>,
        heads: &[(&str, u8)],
        bases: &[(&str, u8)],
        tags: &[(&str, &[(u64, u8)])],
    ) -> (RemoteHeads, ActiveVersionTags) {
        let mut observed =
            observed_ids.into_iter().map(|id| (id, BTreeMap::new())).collect::<HashMap<_, _>>();
        tags.iter().for_each(|(id, tags)| {
            let history = observed
                .get_mut(&change_id(id))
                .unwrap_or_else(|| panic!("test history for unobserved change {id}"));
            *history = versions(tags);
        });
        (
            RemoteHeads::for_test(
                DefaultBranch::new("main".to_owned(), object_id(1)).unwrap(),
                heads.iter().map(|(id, byte)| (change_id(id), object_id(*byte))).collect(),
                bases.iter().map(|(id, byte)| (change_id(id), object_id(*byte))).collect(),
            ),
            ActiveVersionTags::for_test(observed),
        )
    }
    ...
    fn normalizes_only_absent_or_complete_contiguous_publications() {
        assert!(matches!(
            normalize_remote_publication(&change_id("Gone"), None, &versions(&[])).unwrap(),
            RemotePublication::Absent
        ));

        let repeated = versions(&[(1, 2), (2, 2), (3, 3)]);
        let RemotePublication::Published(history) =
            normalize_remote_publication(&change_id("Gone"), Some(object_id(3)), &repeated)
                .unwrap()
        else {
            panic!("complete history must be published");
        };
        assert_eq!(history.iter().collect::<Vec<_>>(), [object_id(2), object_id(2), object_id(3)]);
        ...
        for (head, tags, message) in [
            ...
            let error = normalize_remote_publication(&change_id("Gone"), head, &tags).unwrap_err();
    ...
    fn unchanged_heads_are_no_ops_and_changed_heads_advance_remote_history() {
        let (heads, versions) = remote(
            change_ids(&["Gone", "Gtwo", "Gnew"]),
            &[("Gone", 3), ("Gtwo", 5)],
            &[],
        ...
        let plan = plan_git_publication(&stack, &heads, &versions).unwrap();
    ...
    fn stacks_larger_than_the_removed_query_batch_are_planned_together() {
        ...
        let (heads, versions) = remote(ids.iter().cloned(), &[], &[], &[]);
    ...
    fn every_local_change_is_validated_before_a_plan_exists() {
        let (heads, versions) =
            remote(change_ids(&["Gvalid", "Gbad"]), &[("Gbad", 4)], &[], &[("Gbad", &[(1, 3)])]);
        let stack = stack([(change_id("Gvalid"), object_id(2)), (change_id("Gbad"), object_id(4))]);
        let error = plan_git_publication(&stack, &heads, &versions).unwrap_err();

        assert!(error.to_string().contains("latest version tag"), "error={error:?}");
    }

    #[test]
    fn old_publication_rejects_an_owned_base_for_an_active_local_change() {
        let (heads, versions) = remote(change_ids(&["Gone"]), &[], &[("Gone", 1)], &[]);
    ...
    #[test]
    fn planning_requires_observed_version_history_even_when_no_tags_exist() {
        let (heads, versions) = remote(std::iter::empty(), &[], &[], &[]);
        let stack = stack([(change_id("Gone"), object_id(2))]);
