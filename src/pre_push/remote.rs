...
impl<'local> ExactLocalQueryPlan<'local> {
    ...
    async fn observe<'repository, 'destination>(
        ...
        for (index, query) in queries.iter().enumerate() {
            ...
            let mut command = destination.ls_remote(["--quiet".to_owned()], patterns);
            ...
            if !output.status().success() {
                bail!(
                    "Exact Git observation failed for GHerrit remote '{}' ({} bytes of diagnostics suppressed)",
                    destination.configured_remote(),
                    output.stderr_bytes()
                );
            }
...
async fn acquire(
    ...
    if !output.status().success() {
        bail!(
            "Exact Git history acquisition failed for GHerrit remote '{}' ({} bytes of diagnostics suppressed)",
            destination.configured_remote(),
            output.stderr_bytes()
        );
    }
...
fn prepare_fetch_input(root: &ExternalGraphRoot) -> Result<RegularFileStdin> {
    ...
    // The total size is validated before the anonymous file is created. The
    // builder is the sole caller-side writable handle and seals itself before
    // ownership crosses the subprocess boundary.
...
fn parse_query<'id>(
    ...
    for record in records(output) {
        ...
        if record.peeled {
            bail!("exact local observation returned an unrelated peeled ref");
        }
...
mod tests {
    ...
    fn real_fixture() -> RealFixture {
        ...
        RealFixture { context, remote, external_parent, external }
    }
    ...
    fn write_hostile_config(root: &Path) -> HostileConfig {
        let system = root.join("system.config");
        let global = root.join("global.config");
        let bundle = root.join("missing.bundle");
        ...
        HostileConfig { system, global, bundle, traces, system_contents, global_contents }
    }
    ...
    fn object_exists(context: &testutil::TestContext, oid: ObjectId) -> bool {
        ...
        context.git_cmd().args(["cat-file", "-e", &object]).output().unwrap().status.success()
    }
    ...
    fn assert_hostile_state(hostile: &HostileConfig, remote: &Path) {
        ...
        assert!(!hostile.bundle.exists(), "configured bundle source was created or consumed");
        for trace in &hostile.traces {
            ...
            assert_path_absent(&text, remote);
    ...
    fn assert_local_state_unchanged(
        ...
        assert_hostile_state(hostile, &fixture.remote);
    }
    ...
    fn real_complete_graph_skips_acquisition() {
        ...
        let hostile = write_hostile_config(fixture.context.dir.path());
    ...
    fn validated_tail_matches_are_ignored_without_weakening_absence() {
        ...
        assert!(decode(&ids, [peeled_noise.as_bytes()]).is_err());
    }
