...
impl RepositoryBinding {
    fn new(repository: &util::Repo) -> Result<Self> {
        ...
        // This is also the identity used by hook recursion suppression. Using
        // the same canonical path for the child binding and its marker makes
        // the marker a property of the exact repository Git will open.
        let git_dir = repository.git_dir_identity().as_path().to_owned();
...
impl PushDestination {
    ...
    /// private local path. Exactly one URL and push URL and only the observed
    /// optional proxy inputs are added; no synthesized empty reset values or
    /// Git-version-dependent additive behavior participate.
    ...
    pub(super) fn push(
        ...
        command
            ...
            .env(INTERNAL_PRE_PUSH_GIT_DIR_ENV, self.repository.git_dir.as_os_str());
        command
    }
...
/// destination remains in the child environment alongside `DESTINATION_ENV`
/// rather than entering its argument list.
...
mod tests {
    ...
    fn repository_binding_models_normal_linked_and_bare_repositories() {
        ...
        assert_repository_binding(
            &bare_repository,
            bare_repository.git_dir_identity().as_path(),
            None,
            &hostile,
        );
    }
