...
impl PushDestination {
    ...
    pub(super) fn resolve(configured_remote: util::RemoteName) -> Result<Self> {
        ...
        let transport = resolved.inspect_remote_transport_settings(&probe_remote)?;
...
impl ResolvedDestination {
    ...
    fn inspect_remote_transport_settings(
        ...
        Ok(RemoteTransportSettings {
            proxy: self.inspect_remote_transport_setting(probe_remote, "proxy")?,
            proxy_auth_method: self
                .inspect_remote_transport_setting(probe_remote, "proxyAuthMethod")?,
        })
    }
...
mod tests {
    ...
    fn remote_transport_settings_process_fixture() {
        ...
        let settings = destination
            .inspect_remote_transport_settings(PROBE_REMOTE_STEM)
    ...
    fn configured_trace2_targets_do_not_observe_a_destination_command() {
        ...
        let destination = PushDestination {
            resolved: ResolvedDestination::from_git_output(
                remote(),
                format!("{literal}\n").as_bytes(),
            )
            .unwrap(),
