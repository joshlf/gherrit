...
/// and so on. Adjacent equal revisions cannot be produced by the publication
...
impl PublishedHistory {
    fn from_revisions(mut revisions: impl Iterator<Item = Revision>) -> Result<Self> {
        ...
        let later = revisions
            .scan(first, |previous, revision| {
                let adjacent_duplicate = *previous == revision;
                *previous = revision;
                Some((adjacent_duplicate, revision))
            })
            .map(|(adjacent_duplicate, revision)| {
                if adjacent_duplicate {
                    bail!(
                        "Published history contains adjacent versions of the same literal revision"
                    );
                }
                Ok(revision)
            })
...
impl CommitGraphEvidence {
    ...
    ///
    /// This adapter exists only for pure semantic tests. Its inputs are raw
    /// commit facts, not normalized histories or planning decisions, so the
    /// production validators still establish every publication invariant.
    #[cfg(test)]
    pub(super) fn from_literal_commits_for_test(
        commits: impl IntoIterator<Item = (ObjectId, Vec<ObjectId>, Vec<Vec<u8>>)>,
    ) -> Result<Self> {
        ...
        for (oid, parents, identities) in commits {
            ...
            if !identities.is_empty() {
                trailer_identities.insert(oid, identities.into_boxed_slice());
            }
        }
...
mod tests {
    ...
    #[test]
    fn derives_versions_and_preserves_only_nonconsecutive_repeats() {
        let repository = TestRepository::new();
        let root = repository.commit("root", &[], &[]);
        ...
        let history = change_history(&id, &graph, &[(1, a), (2, b), (3, a)], a)
            .validate(&graph, None)
        ...
        let error = normalize(&id, Some(a), Some(root), &tags([(1, a), (2, a)]), &graph)
...
graph/history pairs: 49934
pairs with merge parents: 47202
histories by length: 1=5574, 2=12700, 3=31660
nonconsecutive A,B,A histories: 12700
accepted: 16608
rejected by identity: 33326
rejected by reachability: 24728
"###);
    }
}
