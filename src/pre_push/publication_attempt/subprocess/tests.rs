...
//!
//! Re-execution is the final test-only adapter until #384 replaces it; every
//! fixture clears its environment and restores only documented bootstrap data.
...
async fn regular_file_stdin_is_rewound_and_can_exceed_pipe_capacity() {
    ...
    assert!(output.status().success());
    assert!(output.stdout() == expected.stdout);
}

#[tokio::test(flavor = "current_thread")]
async fn regular_file_stdin_accepts_the_exact_limit_and_rejects_one_byte_over() {
    let bytes = b"12345678";
    let mut exact = RegularFileStdinBuilder::with_limit(bytes.len() as u64).unwrap();
    exact.write_all(bytes).unwrap();
    let output =
        output_with_regular_file_stdin(reexec("copy-stdin"), exact.finish().unwrap(), TEST_TIMEOUT)
            .await
            .unwrap();
    let expected = copy_stdin_output(bytes);
    assert!(output.status().success());
    ...
    let mut over = RegularFileStdinBuilder::with_limit(bytes.len() as u64).unwrap();
    over.write_all(bytes).unwrap();
    let error = over.write_all(b"9").unwrap_err();
...
async fn job_configuration_failure_does_not_start_the_command() {
    ...
    let error = output_with_faults(command, TEST_TIMEOUT, STDOUT_LIMIT, faults).await.unwrap_err();
...
async fn assert_windows_startup_cleanup(stage: PlatformFaultStage) {
    ...
    let task = tokio::spawn(output_with_faults(command, TEST_TIMEOUT, STDOUT_LIMIT, faults));
    let process_id =
...
async fn startup_timeout_immediately_before_resume_never_runs_the_child() {
    ...
    let task =
        tokio::spawn(output_with_faults(command, Duration::from_millis(50), STDOUT_LIMIT, faults));
    let process_id =
        tokio::task::spawn_blocking(move || spawned.recv_timeout(Duration::from_secs(2)).unwrap())
            .await
            .unwrap();
...
async fn leader_exit_observed_at_the_execution_deadline_fails() {
    ...
    let error = output_with_faults(reexec("nonzero"), TEST_TIMEOUT, STDOUT_LIMIT, faults)
        .await
        .unwrap_err();
...
async fn termination_failure_is_resolved_by_exact_empty_boundary_evidence() {
    ...
    let output =
        output_with_faults(reexec("nonzero"), TEST_TIMEOUT, STDOUT_LIMIT, faults).await.unwrap();
...
async fn boundary_observation_failure_is_returned_after_cleanup() {
    ...
    let error =
        output_with_faults(reexec("sleep"), Duration::from_millis(50), STDOUT_LIMIT, faults)
            .await
            .unwrap_err();
...
async fn stdout_reader_startup_failure_cleans_up_the_spawned_child() {
    ...
    let error =
        output_with_faults(reexec("sleep"), TEST_TIMEOUT, STDOUT_LIMIT, faults).await.unwrap_err();
...
impl ProcessProbe {
    ...
    fn wait_closed(self) {
        ...
        // This accepted connection is an identity-stable lifetime token:
        // only the sleeping descendant owns its peer. EOF therefore
        // proves that exact descendant exited, without consulting a
        // recyclable PID, PGID, or process handle.
        let mut unexpected = [0];
        match connection.read(&mut unexpected) {
            Ok(0) => {}
            Ok(_) => panic!("process probe contained unexpected lifetime data"),
            Err(error) if is_terminal_probe_close(&error) => {}
            Err(error) => panic!("process probe did not reach EOF: {error}"),
        }
    }
}
