...
#[derive(Clone, Debug, Eq, PartialEq)]
struct ObservedOpenRow {
    identity: PullRequestIdentity,
    fields: ObservedOpenFields,
}

/// Completed OPEN-connection results captured before planning.
///
/// An absent change ID observes the current durable OPEN rows. A present entry
/// is the whole response captured earlier, including its membership and an
/// intentionally empty response. Later creates and closes cannot alter that
/// response retroactively. Entries for different change IDs need not describe
/// the same instant, and they do not freeze a later terminal-history probe.
#[derive(Clone, Debug, Default, Eq, PartialEq)]
struct QueryVisibility {
    captured_open: HashMap<GherritPrId, Box<[ObservedOpenRow]>>,
}
...
impl QueryVisibility {
    fn hiding_identities(
        ...
        assert!(
            hidden.iter().all(|identity| world.exact_open_pull_request(identity).is_some()),
            "only an OPEN row can hide"
        );
        let captured_open = world
            .changes
            .iter()
            .filter(|(_, change)| {
    ...
    fn stale(world: &DurableWorld, ids: impl IntoIterator<Item = GherritPrId>) -> Self {
        ...
        for id in ids {
            let rows = world.current_open_rows(&id);
            assert!(
                visibility.captured_open.insert(id, rows.into_boxed_slice()).is_none(),
    ...
    fn historical_slots(
        ...
        let fields = &mut rows
            ...
            .expect("a historical OPEN observation contains a row")
            .fields;
        ...
        Self { captured_open: HashMap::from([(id, rows.into_boxed_slice())]) }
    }

    fn open(&self, id: &GherritPrId) -> Option<&[ObservedOpenRow]> {
        self.captured_open.get(id).map(AsRef::as_ref)
    }
}
...
impl DurableWorld {
    ...
    fn current_open_rows(&self, id: &GherritPrId) -> Vec<ObservedOpenRow> {
        self.open_pull_requests(id)
            .map(|pull_request| ObservedOpenRow {
    ...
    ///
    /// A captured connection is an earlier completed OPEN result, so neither
    /// later closure nor later creation changes its rows. An uncaptured
    /// connection observes current durable OPEN rows. Keeping only that
    ...
    fn observed_open_rows(
        ...
        visibility.open(id).map_or_else(|| self.current_open_rows(id), <[ObservedOpenRow]>::to_vec)
    }
    ...
    fn plan(
        ...
        assert!(
            visibility.captured_open.iter().all(|(id, rows)| {
    ...
    fn expected_duplicate_identities(
        ...
        intent
            ...
            .flat_map(|local| {
                ...
                visible
                    .into_iter()
                    .filter(move |row| Some(row.identity.number().get()) != canonical)
    ...
    fn observe_pull_request(
        ...
        let Some(canonical) = visible.first().cloned() else {
        ...
        Ok(LocalPullRequestObservation::Open(
            ManagedOpenPullRequests::for_plan_test(
                id.clone(),
                canonical.identity,
                canonical.fields.head_oid,
                ObservedBase::for_plan_test(canonical.fields.base, canonical.fields.base_oid),
                &canonical.fields.title,
                &canonical.fields.body,
                false,
            )
