...
fn public_multibatch_intent() -> LocalIntent {
    many_bounded_ids_intent(10, 250).with_public_branch(PUBLIC_BRANCH)
}
...
async fn stale_competitor_loses_completed_create_request_and_primary_resumes() {
    ...
    let (primary, competing) = execute_with_competition(
        ...
        primary,
        competing,
        CompetitionBoundary::BetweenBatches { stage: EffectStage::Create, completed_batch: 0 },
        None,
    )
    .await
    .unwrap();
...
async fn fresh_competitor_wins_future_create_requests_and_stale_primary_stops() {
    ...
    assert_eq!(competing.outcome, AttemptOutcome::Acknowledged);
    assert_eq!(primary.trace.creates.len(), 2);
    assert_eq!(competing.trace.creates.iter().map(|batch| batch.len()).collect::<Vec<_>>(), [1, 1]);
    assert!(primary.trace.markers.is_empty() && primary.trace.updates.is_empty());
    assert!(
        world
            .run_attempt(&create_intent, &QueryVisibility::default(), None)
...
async fn competing_publishers_interleave_between_update_requests() {
    ...
    let (primary, competing) = execute_with_competition(
        ...
        primary,
        competing,
        CompetitionBoundary::BetweenBatches { stage: EffectStage::Update, completed_batch: 0 },
        None,
    )
    .await
    .unwrap();
    ...
    let competing_root_body = flatten(&competing.trace.updates)
        .iter()
        .find(|update| update.resolved_id.as_ref() == Some(shared_root))
        .and_then(|update| update.operation.body.clone())
        .unwrap();
    assert_eq!(
    ...
    let primary_resumed_body = flatten(&primary.trace.updates)
        .iter()
        .find(|update| update.resolved_id.as_ref() == Some(resumed_id))
        .and_then(|update| update.operation.body.clone())
        .unwrap();
    assert_eq!(
