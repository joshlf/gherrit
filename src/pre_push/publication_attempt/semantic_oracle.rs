...
enum PullRequestState {
    Open { title: String, body: String, base: BaseKind },
    Closed,
    Merged,
}
...
impl PullRequest {
    fn open_fields(&self) -> Option<(&str, &str, BaseKind)> {
        match &self.state {
            PullRequestState::Open { title, body, base } => Some((title, body, *base)),
...
impl DurableWorld {
    ...
    fn reopen_for_setup(
        ...
        pull_request.state =
            PullRequestState::Open { title: title.to_owned(), body: body.to_owned(), base };
        self.assert_well_formed();
    }
        ...
        change
            ...
            .map(|pull_request| {
                let state = match &pull_request.state {
                    PullRequestState::Open { title, body, base } => {
