...
/// Returns exactly the update aliases which a fresh observation must still
/// plan after one response became indeterminate.
fn unapplied_updates(
    complete: &AttemptTrace,
    interrupted_batch: usize,
    applied_aliases: &[usize],
) -> Box<[UpdateEffect]> {
    let interrupted = complete
        .updates
        .get(interrupted_batch)
        .expect("the interruption selects a complete update batch");
    ...
    for (index, interruption) in interruptions.into_iter().enumerate() {
        ...
        if let Interruption::Update { batch, applied_aliases } = &interruption {
            ...
            assert_eq!(
                ...
                unapplied_updates(&report.trace, *batch, applied_aliases),
