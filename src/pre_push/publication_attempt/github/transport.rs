...
mod tests {
    ...
    #[test]
    fn production_client_rejects_a_different_forge_with_matching_coordinates() {
    ...
    #[tokio::test]
    async fn a_one_alias_oversized_query_document_is_rejected_without_a_request() {
        let (api_url, server) = scripted_peer(Vec::new()).await;
        let oversized_id = "G".repeat(MAX_GRAPHQL_QUERY_BYTES);
        let error = test_github(&api_url, test_timeouts())
            .observe_local_pull_requests(&local(&[&oversized_id]))
            .await
            .unwrap_err();
        let requests = finish_peer(server).await;
