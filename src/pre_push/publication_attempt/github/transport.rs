...
impl Github {
    ...
    /// Exhausts the OPEN pull request connection for each sealed local change.
...
mod tests {
    ...
    #[test]
    fn production_client_rejects_destinations_outside_supported_github_push_transports() {
        ...
        for destination in [
            ...
            let error = Github::new(&GithubEndpoint::Production, &other)
                .err()
                .expect("an unsupported GitHub transport must fail before client construction");
            assert_eq!(
                error.to_string(),
    ...
    async fn a_mutation_attempt_timeout_is_indeterminate_and_never_replayed() {
        ...
        let error =
            test_github(&api_url, timeouts).project_pull_requests(projection).await.unwrap_err();
        let requests = finish_peer(server).await;
