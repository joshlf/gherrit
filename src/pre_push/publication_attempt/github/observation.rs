...
///
/// Cross-repository and terminal variants retain only identity authority.
/// The exact requested head was already checked and therefore cannot disagree
/// with a separately stored field. Only OPEN rows retain projection payload.
#[derive(Debug)]
enum DecodedPullRequest {
    CrossRepository { identity: PullRequestIdentity },
    Terminal { identity: PullRequestIdentity, state: TerminalState },
    Open(OpenPullRequest),
}

impl DecodedPullRequest {
    fn identity(&self) -> &PullRequestIdentity {
        match self {
            Self::CrossRepository { identity } | Self::Terminal { identity, .. } => identity,
            Self::Open(pull_request) => &pull_request.identity,
        }
    }
}
...
fn decode_pull_request(id: &GherritPrId, node: Node) -> Result<DecodedPullRequest> {
    ...
    if node.is_cross_repository {
        return Ok(DecodedPullRequest::CrossRepository { identity });
    }
...
impl LocalPullRequestAccumulator {
    ...
    fn record_page(
        ...
        if let Some(row) = row {
            ...
            self.identities.insert_observation(row.identity())?;
            match row {
                ...
                DecodedPullRequest::Open(_) if connection.open.is_some() => bail!(
                    "GitHub has more than one same-repository OPEN pull request for '{}'",
                    diagnostic_detail(id.as_str())
                ),
                DecodedPullRequest::Open(open) => connection.open = Some(open),
            }
        }
...
mod tests {
    ...
    fn one_id_accepts_its_baseline_plus_exactly_99_excess_rows() {
        ...
        assert_eq!(complete.identities.lengths(), (100, 100));
    }
        ...
        let batch = LocalPullRequestBatch {
            ...
            pages: vec![
                LocalPullRequestPageEvidence {
                    id: id("C"),
                    after: None,
                    end: PageEnd::Exhausted {
                        row: Some(DecodedPullRequest::CrossRepository {
                            identity: PullRequestIdentity::new(2001, "NODE-C-0".to_owned())
                                .unwrap(),
                        }),
                    },
                },
                LocalPullRequestPageEvidence {
                    ...
                    after: Some("cursor-B-1000".to_owned()),
                        ...
                        row: Some(DecodedPullRequest::CrossRepository {
                            identity: PullRequestIdentity::new(1002, "NODE-B-1001".to_owned())
                                .unwrap(),
                        }),
                    },
                },
    ...
    #[test]
    fn open_wins_history_and_forks_while_every_identity_is_retained() {
        ...
        assert_eq!(complete.identities.number_values(), HashSet::from([1, 2, 3]));
        assert_eq!(
            complete.identities.node_id_values(),
            HashSet::from(["TERMINAL", "FORK", "OPEN"])
        );
    }
    ...
    fn terminal_only_diagnostics_are_globally_bounded() {
        let ids = (0..MAX_DIAGNOSTIC_IDENTITIES + 7)
            .map(|index| id(&format!("G{}{}", index, "x".repeat(500))))
            .collect::<Vec<_>>();
...
"###);
    ...
    fn identity_number_node_and_pair_collisions_reject_across_ids_and_pages() {
        for (second_number, second_node) in [(1, "TWO"), (2, "ONE"), (1, "ONE")] {
            let first = decoded_page("A", None, Some(fork(1, "ONE", "A")), None, true);
            let second =
                decoded_page("B", None, Some(fork(second_number, second_node, "B")), None, false);
            assert!(
        ...
        for (second_number, second_node) in [(1, "TWO"), (2, "ONE"), (1, "ONE")] {
            let first = decoded_page("A", None, Some(fork(1, "ONE", "A")), Some("next"), true);
            let second = decoded_page(
                "A",
                Some("next"),
                Some(fork(second_number, second_node, "A")),
                None,
                false,
            );
            assert!(
