...
mod tests {
    ...
    async fn captured_empty_public_stack_projects_unicode_c1_to_the_exact_default_tip() {
        ...
        assert_eq!(observed.publish(&repository, &GithubEndpoint::Production).await.unwrap(), 0);
    ...
    async fn production_rejects_a_filesystem_destination_before_remote_observation() {
            ...
            [("empty-public", true, false), ("nonempty-private", false, true)]
        {
            ...
            context
                ...
                .success();
            let repository = util::Repo::open(context.repo_path.to_str().unwrap()).unwrap();
            let invocation = Invocation::new(None, None).unwrap();
            let error = run(&repository, &GithubEndpoint::Production, invocation)
                .await
                .unwrap_err()
                .to_string();
    ...
    async fn empty_public_transition_stops_if_the_named_default_ref_moves() {
        ...
        context
            ...
            .success();
        let error = observed
            .publish(&repository, &GithubEndpoint::Production)
            .await
            .unwrap_err()
            .to_string();
