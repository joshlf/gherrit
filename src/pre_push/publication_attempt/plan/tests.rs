...
fn repository_default_and_proposal_facts_must_match() {
    ...
    histories[0] =
        ValidatedChangeHistory::for_plan_test(id("Gone"), &[], (oid(99), oid(10)), false);
    let error = plan_effects(&destination, None, stack, histories, pull_requests).err().unwrap();
    ...
    let Inputs { destination, stack, mut histories, pull_requests } =
        inputs(std::slice::from_ref(&spec));
    histories[0] =
        ValidatedChangeHistory::for_plan_test(id("Gone"), &[], (oid(20), oid(99)), false);
    let error = plan_effects(&destination, None, stack, histories, pull_requests).err().unwrap();
    assert!(error.to_string().contains("local proposal and first parent"));
}
...
fn every_published_owned_head_and_base_pair_is_independently_valid() {
    ...
    let history =
        ValidatedChangeHistory::for_plan_test(id("Ghistory"), &published, (oid(20), oid(10)), true);
    let default = default_branch(DEFAULT_NAME, oid(10));
...
fn tuple_selection_uses_only_absence_or_a_changed_current_revision() {
    let absent =
        ValidatedChangeHistory::for_plan_test(id("Gabsent"), &[], (oid(20), oid(10)), false);
...
#[test]
fn every_statically_known_stage_is_preflighted_before_a_tuple_plan_can_escape() {
    let fresh = EntrySpec {
        id: "Gfresh",
        history: HistorySpec::absent(),
        pull_request: PullRequestSpec::Absent,
    ...
    let huge_id: &'static str = Box::leak(format!("G{}", "x".repeat(9_000)).into_boxed_str());
    let specs = [
        EntrySpec {
            id: "Gtuple",
        ...
        EntrySpec {
            id: huge_id,
            history: HistorySpec::current(oid(21), oid(20), false),
            pull_request: PullRequestSpec::Open(OpenSpec::new(
                2,
                oid(21),
                BaseKind::Owned,
