...
/// projection.
/// The branch identity has already been proved disjoint from GHerrit's owned
/// namespaces and the repository default branch.
#[derive(Clone, Debug)]
...
impl PreparedPushes {
    ...
    pub(super) async fn execute(self, destination: &PushDestination) -> Result<()> {
        ...
        for batch in self.batches.into_vec() {
            ...
            require_acknowledgement(batch.outcome(output.status(), output.stdout()))?;
        }
        Ok(())
    }
...
fn require_acknowledgement(outcome: PushOutcome) -> Result<()> {
    match outcome {
        PushOutcome::Acknowledged => Ok(()),
        PushOutcome::Rejected => bail!(
            "Git publication was rejected without changing the requested refs; stop this attempt and run 'git push' again after resolving the conflict"
        ),
        PushOutcome::Indeterminate => bail!(
            "Git publication acknowledgement is indeterminate; requested refs may or may not have changed, so stop this attempt and run 'git push' again to reobserve them"
        ),
    }
}
...
fn parse_push_receipts(
    ...
    let output = str::from_utf8(stdout).ok()?;
    let (output, line_ending) = output
        .strip_suffix("\r\n")
        .map(|output| (output, "\r\n"))
        .or_else(|| output.strip_suffix('\n').map(|output| (output, "\n")))?;
    let (body, footer) = output.rsplit_once(line_ending)?;
    let (header, status_lines) = body.split_once(line_ending)?;
    parse_push_receipt_lines(expected, header, footer, status_lines.split(line_ending))
}
...
mod tests {
    ...
    fn public_branch(value: &str) -> PublicBranch {
        PublicBranch::for_test(value).unwrap()
    }
    ...
    #[test]
    fn public_branch_is_the_last_unit_of_the_initial_git_stage() {
    ...
    fn only_exact_acknowledgement_releases_the_next_stage() {
        require_acknowledgement(PushOutcome::Acknowledged).unwrap();

        let rejected = require_acknowledgement(PushOutcome::Rejected).unwrap_err();
        assert!(rejected.to_string().contains("without changing the requested refs"));
        assert!(!rejected.to_string().contains("may or may not"));

        let indeterminate = require_acknowledgement(PushOutcome::Indeterminate).unwrap_err();
        assert!(indeterminate.to_string().contains("may or may not have changed"));
    }
