...
fn parse_push_receipts(
    ...
    // be Git's destination header. Any additional status record therefore
    // becomes the would-be header and is rejected.
    let header = split_last(body, line_ending).map_or(body, |(_, header)| header);
