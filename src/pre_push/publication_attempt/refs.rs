...
use super::{marker::PreparedMarker, version::Version};
use crate::pre_push::{
...
impl MarkerTransition {
    /// Converts a preflighted immutable tag object into a create-only ref
    /// transition. Equal templates acknowledge the same tag object, while a
    /// different pull-request number conflicts with the absence lease.
...
fn parse_push_receipts(
    ...
    // its own translated header. Retain only the final header line instead of
    // assuming that the independent producer ended with LF. The fixed receipt
    // suffix still makes any extra status record the would-be header, whose
    // tab bytes the framing checks below reject. Header and footer text are
    // deliberately opaque so the push can preserve the caller's locale.
    let header = body.rsplit(|byte| *byte == b'\n').next()?;
...
fn parse_push_receipt_lines<'line>(
    ...
    header: &[u8],
    footer: &[u8],
    status_lines: impl IntoIterator<Item = &'line [u8]>,
) -> Option<Vec<(ExpectedRefTransition, ReceiptStatus)>> {
    let valid_framing =
        |line: &[u8]| !line.is_empty() && !line.iter().any(|byte| byte.is_ascii_control());
    if !valid_framing(header) || !valid_framing(footer) {
        return None;
    }
    ...
    for line in status_lines {
        ...
        let separator = refs.iter().position(|byte| *byte == b':')?;
        let (source, destination) = (&refs[..separator], &refs[separator + 1..]);
        let destination = str::from_utf8(destination).ok()?;
        let expected_ref = expected.refs.get(destination)?;
        if source != expected_ref.source.to_string().as_bytes()
            || summary.is_empty()
            || !destination.starts_with("refs/")
            || summary.iter().any(|byte| byte.is_ascii_control())
...
mod tests {
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn identical_prepared_tuples_execute_against_their_bound_real_repository() {
        ...
        // Seal both create actions from the same absent state before either
        // push can change the remote.
        let first = prepare_tuple_pushes(
            &destination,
        ...
        .unwrap();
        let second = prepare_tuple_pushes(
            &destination,
            &[TupleTransition::create(
                id("Gpublication"),
                PublicationRevision::new(head, base).unwrap(),
        ...
        first.execute(&destination).await.unwrap();
        second.execute(&destination).await.unwrap();
    ...
    fn receipt_decoder_stops_after_the_first_impossible_extra_record() {
        ...
        let poison = std::iter::from_fn(|| {
            polls += 1;
            match polls {
                1 | 2 => Some(status.as_bytes()),
                _ => panic!("receipt parsing consumed past the first extra status record"),
            }
        });

        assert!(parse_push_receipt_lines(&expected, b"To private", b"Done", poison).is_none());
    }
    ...
    fn receipt_decoder_uses_the_exact_final_block_after_composite_hook_output() {
        ...
        let translated_header = format!("policy check passedprivate\n{receipt}\nDone\n");
        assert!(receipts_acknowledge(&expected, translated_header.as_bytes()));

        let translated = format!("Vers destination privée\n{receipt}\nTerminé\n");
        assert!(receipts_acknowledge(&expected, translated.as_bytes()));

        let mut non_utf8_framing = b"destination:\xff\n".to_vec();
        non_utf8_framing.extend(receipt.as_bytes());
        non_utf8_framing.extend_from_slice(b"\ncomplete:\xfe\n");
        assert!(receipts_acknowledge(&expected, &non_utf8_framing));

        let extra = format!("opaque header\n{receipt}\n{receipt}\nopaque footer\n");
        assert!(!receipts_acknowledge(&expected, extra.as_bytes()));
    }
