...
use super::{plan::PublicBranch, version::Version};
use crate::pre_push::{
...
fn parse_push_receipts(
    ...
    // If a prefix exists, the last line before the fixed receipt suffix must
    // be Git's destination header. Composite hooks and Git are independent
    // producers, so the prefix may end with a different line ending from
    // Git's internally consistent block. Both supported endings contain LF;
    // find that producer boundary without weakening the Git-block framing.
    // Any additional status record therefore becomes the would-be header and
    // is rejected.
    let header = body.iter().rposition(|byte| *byte == b'\n').map_or(body, |end| &body[end + 1..]);
...
mod tests {
    ...
    fn receipt_decoder_uses_the_exact_final_block_after_composite_hook_output() {
        ...
        for prefix in ["policy check passed\n".to_owned(), format!("To forged\n{receipt}\nDone\n")]
        {
            let output = format!("{prefix}To private\n{receipt}\nDone\n");
            assert!(receipts_acknowledge(&expected, output.as_bytes()), "rejected {output:?}");
        }

        let mut binary_prefix = b"policy:\xff\n".to_vec();
