...
/// Decodes direct records from a provider whose requested heads and tags are
/// known to be absent or direct.
///
/// `ls-remote` omission is not authoritative for an arbitrary Git server: a
/// hidden or dangling symbolic ref can look direct or absent. The production
/// entrypoint validates GitHub.com before remote I/O, and custom test
/// destinations promise the same fixture property. Explicitly advertised
/// symbolic records are still rejected below.
...
mod tests {
    ...
    fn assert_hostile_state(fixture: &RealFixture, hostile: &HostileConfig) {
        ...
        for trace in &hostile.traces {
            ...
            assert!(!bytes.is_empty(), "the Trace2 probe must emit evidence");
