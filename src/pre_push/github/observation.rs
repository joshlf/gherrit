...
use super::{
    ObservedPullRequest, PullRequestIdentity, PullRequestState, Repository, RepositoryNodeId,
};
use crate::pre_push::{
...
struct LocalPullRequestAccumulator {
    ...
    rows: HashMap<GherritPrId, Vec<ObservedPullRequest>>,
...
impl LocalPullRequestAccumulator {
    ...
    fn record_page(&mut self, page: LocalPullRequestPageEvidence) -> Result<()> {
        ...
        self.rows.entry(id.clone()).or_default().extend(pull_requests);
...
/// Repository facts and complete rows for the exact requested local IDs.
#[derive(Debug)]
struct CompleteLocalPullRequests {
    repository: Repository,
    entries: Box<[(GherritPrId, Vec<ObservedPullRequest>)]>,
}
...
mod tests {
    ...
    #[test]
    fn complete_rows_retain_every_authority_field_and_opaque_body() {
        ...
        assert_eq!(rows[0].head_branch, "Gone");
