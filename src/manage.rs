...
fn get_expected_config(state: State, branch_name: &str, default_remote: &str) -> BranchConfig {
    let self_merge_ref = format!("refs/heads/{branch_name}");
    match state {
        State::Unmanaged => BranchConfig {
            push_remote: None,
            remote: None,
            merge: None,
        },
        State::Private => BranchConfig {
            push_remote: Some(".".to_string()),
            remote: Some(".".to_string()),
            merge: Some(self_merge_ref),
        },
        State::Public => BranchConfig {
            push_remote: Some(default_remote.to_string()),
            remote: Some(".".to_string()),
            merge: Some(self_merge_ref),
        },
    }
...
pub fn read_actual_config(repo: &util::Repo, branch_name: &str) -> Result<BranchConfig> {
...
pub fn set_state(repo: &util::Repo, new_state: State, force: bool) -> Result<()> {
    ...
    let expected_old_config = get_expected_config(old_state, branch_name, &default_remote);

    // Step 2: Check for Drift
    let current_config = read_actual_config(repo, branch_name)?;
    if !force && current_config != expected_old_config {
        log::warn!(
            "Configuration drift detected for branch '{}'.",
            branch_name.yellow()
        );
        log::warn!(
            "The current git configuration does not match the expected state for '{:?}'.",
            old_state
        );
        log::warn!("Current: {:?}", current_config);
        log::warn!("Expected: {:?}", expected_old_config);
    ...
    let new_config = get_expected_config(new_state, branch_name, &default_remote);
