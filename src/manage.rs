...
enum TransitionPreparation {
    /// The plan is complete without reading current branch configuration.
    Planned(TransitionPlan),
    /// Current branch configuration must be observed before planning can finish.
    NeedsConfig(ConfigTransition),
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
enum TransitionPlan {
    /// The branch is already explicitly unmanaged, so no write is required.
    NoChange,
    /// Refuse to overwrite configuration drift when `--force` was not supplied.
    BlockedByDrift { differences: Vec<ConfigDifference> },
    /// Write the requested state and the minimal required configuration changes.
    ///
    /// `overwritten_drift` contains user changes whose replacement was
    /// authorized by `--force`. This includes a same-state managed transition,
    /// which can be used to restore GHerrit's expected configuration.
    Apply {
        state: State,
...
fn prepare_transition(request: TransitionRequest) -> TransitionPreparation {
    ...
    match (current_state, requested_state) {
        (Some(State::Unmanaged), State::Unmanaged) => {
            TransitionPreparation::Planned(TransitionPlan::NoChange)
        }
        ...
        (None, State::Unmanaged) => TransitionPreparation::Planned(TransitionPlan::Apply {
        ...
        _ => {
            ...
            TransitionPreparation::NeedsConfig(ConfigTransition {
                expected_current,
                desired,
                requested_state,
                force,
...
impl ConfigTransition {
    fn plan(self, current_config: BranchConfig) -> TransitionPlan {
        ...
        if !differences.is_empty() && !self.force {
            return TransitionPlan::BlockedByDrift { differences };
        }
...
pub fn set_state(repo: &util::Repo, new_state: State, force: bool) -> Result<()> {
    ...
    let plan = match preparation {
        TransitionPreparation::Planned(plan) => plan,
        TransitionPreparation::NeedsConfig(transition) => {
    ...
    let state = match plan {
        ...
        TransitionPlan::BlockedByDrift { differences } => {
...
mod tests {
    ...
    fn unmanaged_no_op_and_intent_recording_need_no_config_observation() {
        [false, true].into_iter().for_each(|force| {
            ...
            assert_eq!(
                ...
                TransitionPreparation::Planned(TransitionPlan::Apply {
                    state: State::Unmanaged,
                    config_updates: Vec::new(),
                    overwritten_drift: Vec::new(),
                })
                ...
                [false, true].into_iter().for_each(|force| {
                    ...
                    assert_eq!(
                        matches!(
                            ...
                            TransitionPreparation::NeedsConfig(_)
                        ),
                        !special,
    ...
    fn config_transition_policy_is_exhaustive_and_minimal() {
        ...
        STATES.into_iter().for_each(|current_state| {
            REQUESTED_STATES.into_iter().for_each(|requested_state| {
                [false, true].into_iter().for_each(|force| {
                    let TransitionPreparation::NeedsConfig(transition) =
                        prepare_transition(request(current_state, requested_state, force))
                    else {
                        return;
                    };
