...
pub fn set_state(repo: &util::Repo, new_state: State, force: bool) -> Result<()> {
    ...
    match transition_kind(old_state, new_state) {
        ...
        TransitionKind::ReconcileConfig => {
            ...
            let (expected, decision) = match expected {
                Ok(expected) => {
                    let decision = drift_decision(&current, &expected, force);
                    (expected, decision)
                }
                Err(_)
                    if force
                        && old_state == Some(State::Public)
                        && matches!(new_state, State::Private | State::Unmanaged) =>
                {
                    (current.clone(), DriftDecision::Overwrite)
                }
