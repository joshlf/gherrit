...
fn resolve_hooks_dir(repo: &Repo) -> Result<PathBuf> {
    // Use `git rev-parse --git-path hooks` to resolve the correct hooks directory.
    // This handles `core.hooksPath` configuration and worktrees correctly.
    ...
    // Ensure the hooks directory is within the repository, as per user requirement.
    // This prevents accidental modification of global hooks.
