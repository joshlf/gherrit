...
//! harness places a small `git` wrapper on `PATH`; that wrapper invokes this
//! binary with [`INVOCATION`] before forwarding Git's arguments.
...
pub fn is_invocation() -> bool {
    env::args().nth(1).as_deref() == Some(INVOCATION)
}

pub fn run() -> ExitCode {
    let args = std::iter::once("git".to_string()).chain(env::args().skip(2)).collect::<Vec<_>>();
