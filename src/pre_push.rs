...
impl PrBodyBuilder<'_> {
    fn build(self) -> String {
        ...
        write_body(&self, &mut full_size, HistoryTableFormat::Full).unwrap();
        let history_table_format = if full_size.0 > 65536 {
            HistoryTableFormat::Truncated
        } else {
    ...
    fn write_history_table(&self, mut w: impl Write) -> fmt::Result {
        if self.latest_version > 1 && !self.repo_url.is_empty() {
            ...
            // Rows
            for v_row in (1..=self.latest_version).rev() {
                w.write_str(&format!("|v{}|", v_row))?;
                ...
                let base_link = format!(
                    ...
                    self.repo_url,
                    self.base_branch,
                    self.child_id.unwrap_or_default(),
                    v_row
                );
                w.write_str(&format!(" {} |", base_link))?;

                // Previous version columns
                for v_col in 1..self.latest_version {
                    if v_col < v_row {
                        let link = format!(
