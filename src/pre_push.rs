...
async fn sync_prs(
    ...
    // TODO: Consider processing pages in parallel if it provides a performance
    // benefit.
    let prs = octocrab
        .all_pages(prs_page)
        .await
        ...
        .map(|(c, parent_branch)| {
            ...
            if let Some(pr) = pr_info {
                ...
                Ok(PrResolution::Existing(PrState {
                    number: pr_num,
                    // TODO: If we could resolve the repo's owner and name
                    // earlier, we could avoid needing to parse this from the
                    // response and instead just construct URLs ourselves from
                    // the owner, repo name, and PR number. That would simplify
                    // logic below as well:
                    //
                    ...
                    url: pr
                        ...
                        .clone()
                        .map(|u| u.to_string())
                        .ok_or_else(|| eyre!("Missing URL for PR #{pr_num}"))?,
                    node_id: pr
                        .node_id
                        .clone()
    ...
    let new_prs = if num_creations > 0 {
        log::info!("Batch creating {num_creations} PRs...");
    ...
    for (i, (c, parent_branch, pr_state)) in commit_pr_states.iter().enumerate() {
        ...
        if pr_state.base != parent_branch.as_str() {
            changed = true;
        }
...
struct BatchUpdate {
    ...
    title: String,
    body: String,
    /// The name of the base branch (e.g., "main").
    base_branch: String,
}
...
async fn batch_create_prs(
    ...
    for chunk in creations.into_iter().collect::<Vec<_>>().chunks(50) {
        let mut mutation_body = String::new();
