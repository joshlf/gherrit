...
async fn sync_prs(
    ...
    let prs_page = octocrab
        ...
        .list()
        // Filter by open PRs to match `gh pr list` behavior.
        // We filter for matching head refs client-side.
    ...
    // Use sequential iteration for network operations to avoid complexity.
