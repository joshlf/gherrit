...
/// Runs GHerrit using the current process's arguments and environment.
pub fn run() -> ExitCode {
    #[cfg(gherrit_test)]
...
fn dispatch() -> Result<()> {
    ...
    // A library host may already own Rayon's global pool. In that case, use
    // the host's configuration rather than panicking during command dispatch.
