...
the local stack. It exhausts open pull requests first and probes terminal
history only for an ID with no same-repository open pull request. This keeps
network and backend work proportional to the local stack and its history
rather than to unrelated repository state. It validates and plans the complete
stack before publishing any prefix.
