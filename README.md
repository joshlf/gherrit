...
Fetch the current default branch, drop the local commit for the merged root,
rebase the remaining commits, and push again. For example, if the configured
GHerrit remote is `origin` and the default branch is `main`, replace
`<merged-root-commit>` with the local commit for the PR which just landed:

```bash
git fetch origin main
git rebase --onto origin/main <merged-root-commit>
git push
```
