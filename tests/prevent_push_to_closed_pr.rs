...
fn test_post_push_checks_closed_pr() {
    ...
    if !ctx.is_live {
        ...
        let pr_idx = state
            .prs
            .iter()
            .position(|p| p.head.ref_field == "gherrit/G35fd0e68d0df60807b55f6515f45812e");

        let pr = if let Some(idx) = pr_idx {
            &mut state.prs[idx]
        } else {
            state.prs.last_mut().expect("PR not found")
        };
    ...
    assert.stderr(predicate::str::contains("Cannot push to a CLOSED PR"));
...
fn test_post_push_checks_merged_pr() {
    ...
    assert.stderr(predicate::str::contains("Cannot push to a MERGED PR"));
}
