...
#[test]
fn credential_bearing_http_destinations_reject_scoped_redirect_keys() {
    for userinfo in ["token-only", "user:password-secret"] {
        let ctx = testutil::test_context!()
            .with_remote()
            .with_initial_commit()
            .with_mock_github()
            .with_git_interceptor()
            .build();
        let server = ctx.mock_server_url();
        let authority = server.strip_prefix("http://").unwrap();
        let destination = format!("http://{userinfo}@{authority}/owner/repo.git");
        ctx.set_config("remote.origin.pushurl", Some(&destination));
        ctx.set_config("http.https://unrelated.invalid/.followRedirects", Some("false"));
        ctx.checkout_managed_private("credential-redirect-configuration");
        ctx.commit_with_gherrit_id("Keep HTTP credentials out of arguments");

        ctx.hook_cmd("pre-push")
            .assert()
            .failure()
            .stderr(predicate::str::contains(
                "URL-scoped Git HTTP redirect configuration is incompatible with credential-bearing",
            ))
            .stderr(predicate::str::contains(userinfo).not())
            .stderr(predicate::str::contains(&destination).not());
...
#[test]
fn credential_bearing_http_destination_without_scoped_keys_reaches_git() {
    let ctx = testutil::test_context!()
        .with_remote()
        .with_initial_commit()
        .with_mock_github()
    ...
    let server = ctx.mock_server_url();
    let authority = server.strip_prefix("http://").unwrap();
    let destination = format!("http://token-secret@{authority}/owner/repo.git");
    ctx.set_config("remote.origin.pushurl", Some(&destination));
    ctx.checkout_managed_private("credential-http-destination");
    ctx.commit_with_gherrit_id("Keep a token out of Git arguments");
...
#[test]
fn a_conditional_include_cannot_hide_a_credential_redirect_key() {
    let ctx = testutil::test_context!()
        .with_remote()
        .with_initial_commit()
        .with_mock_github()
    ...
    let server = ctx.mock_server_url();
    let authority = server.strip_prefix("http://").unwrap();
    let destination = format!("http://user:conditional-secret@{authority}/owner/repo.git");
    ctx.set_config("remote.origin.pushurl", Some(&destination));
    configure_unmatched_fetch_destination(&ctx, "credential-unmatched-fetch.git");

    let include = ctx.dir.path().join("credential-redirect-override.config");
    configure_file(&ctx, &include, "http.https://unrelated.invalid/.followRedirects", "true");
    include_when_remote_url_matches(&ctx, &destination, &include);
    ctx.checkout_managed_private("conditional-credential-redirect");
    ctx.commit_with_gherrit_id("Inspect credential-bearing conditional configuration");
    ...
    ctx.hook_cmd("pre-push")
        ...
        .stderr(predicate::str::contains("conditional-secret").not())
        .stderr(predicate::str::contains(&destination).not());
...
#[test]
fn inherited_git_transport_diagnostics_cannot_persist_credentials() {
    let ctx = testutil::test_context!()
        .with_remote()
        .with_initial_commit()
        .with_mock_github()
    ...
    let server = ctx.mock_server_url();
    let authority = server.strip_prefix("http://").unwrap();
    let destination = format!("http://trace-user:trace-secret@{authority}/owner/repo.git");
    ctx.set_config("remote.origin.pushurl", Some(&destination));
    ctx.checkout_managed_private("private-transport-diagnostics");
    ctx.commit_with_gherrit_id("Do not persist a private transport destination");
    ...
    ctx.hook_cmd("pre-push")
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("trace-secret").not())
        .stderr(predicate::str::contains(&destination).not());
    ...
    let retained = [trace, trace_curl, trace2]
        ...
        .flatten()
        .collect::<Vec<_>>();
    assert!(!retained.windows(b"trace-secret".len()).any(|value| value == b"trace-secret"));
    assert!(!retained.windows(b"trace-user".len()).any(|value| value == b"trace-user"));
    assert!(!String::from_utf8_lossy(&retained).contains(&destination));
}

#[cfg(unix)]
#[test]
fn credential_bearing_file_url_is_private_and_reaches_its_repository() {
    let ctx = testutil::test_context!()
        .with_remote()
        .with_initial_commit()
        .with_mock_github()
        .with_git_interceptor()
        .build();
    let path = configured_url(&ctx, &["remote", "get-url", "origin"]);
    assert!(path.starts_with('/'));
    let destination = format!("file://token:password-secret@localhost{path}");
    ctx.set_config("remote.origin.pushurl", Some(&destination));
    ctx.checkout_managed_private("credential-file-destination");
    let id = ctx.commit_with_gherrit_id("Publish without retaining credentials");
    ...
    assert!(!format!("{:?}", pushes[0]).contains("password-secret"));
}
...
#[test]
fn a_conditional_include_cannot_add_an_internal_push_destination() {
    let ctx = testutil::test_context!()
        .with_remote()
        .with_initial_commit()
        .with_mock_github()
    ...
    let id = ctx.commit_with_gherrit_id("Reset an included push destination");
    let managed_ref = format!("refs/heads/{id}");
...
#[test]
fn a_conditional_include_cannot_configure_the_internal_remote() {
    let ctx = testutil::test_context!()
        .with_remote()
        .with_initial_commit()
        .with_mock_github()
    ...
    ctx.commit_with_gherrit_id("Reject hidden private remote behavior");

    ctx.hook_cmd("pre-push")
        .assert()
        .failure()
        .stderr(predicate::str::contains(
            "Git configuration changes the private publication remote",
        ))
        .stderr(predicate::str::contains(&destination).not())
        .stderr(predicate::str::contains("malicious-receive-pack").not());
