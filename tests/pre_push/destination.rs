...
#[test]
fn destination_scoped_redirect_override_is_rejected_without_disclosure() {
    let ctx = testutil::test_context!()
        .with_remote()
        .with_initial_commit()
        .with_mock_github()
        .with_git_interceptor()
        .build();
    let destination = "https://example.invalid/private-owner/private-repo.git";
    let scoped_key = format!("HtTp.{destination}.FoLlOwReDiReCtS");
    ctx.set_config("remote.origin.pushurl", Some(destination));
    ctx.set_config(&scoped_key, Some("true"));
    ctx.checkout_managed_private("redirect-override");
    ctx.commit_with_gherrit_id("Reject destination-scoped redirects");
