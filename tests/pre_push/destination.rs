...
#[test]
fn private_remote_does_not_activate_conditioned_configuration() {
    let ctx = testutil::test_context!()
        .with_remote()
        .with_initial_commit()
        .with_mock_github()
    ...
    let id = ctx.commit_with_gherrit_id("Keep command-scoped remotes out of include conditions");
    ...
    let pushes = ctx.recorded_pushes();
    assert_eq!(pushes.len(), 2);
    // Git has already evaluated `hasconfig:remote.*.url` before GHerrit's
    // `--config-env` option adds the private remote. That later value cannot
    // make this include active, so the preferred name remains safe to use.
    assert!(pushes.iter().all(|push| recorded_remote(push) == "gherrit-publication"));
    assert!(pushes.iter().all(|push| {
        push.arguments().iter().all(|argument| argument != "unplanned-receive-pack")
    }));
}
