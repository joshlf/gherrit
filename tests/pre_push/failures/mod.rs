...
fn head_query(ctx: &testutil::TestContext) -> testutil::GraphQlOperation {
    testutil::GraphQlOperation::pull_request_query([ctx.gherrit_id("HEAD").unwrap()], true)
}
...
fn test_pre_push_edit_failure() {
    ...
    assert_eq!(
        ...
        [vec![head_query(&ctx)], vec![testutil::GraphQlOperation::UpdatePr],],
...
fn test_pre_push_pr_list_failure() {
    ...
    ctx.assert_failure_consumed();
    assert_eq!(ctx.github().requests(), vec![vec![head_query(&ctx)]]);
    assert!(ctx.github().pull_requests().is_empty());
    assert!(ctx.recorded_pushes().is_empty());
}
...
fn test_pre_push_pr_list_stops_after_three_transient_http_retries() {
    ...
    ctx.assert_failure_consumed();
    assert_eq!(ctx.github().requests(), vec![vec![head_query(&ctx)]; 4]);
    assert!(ctx.github().pull_requests().is_empty());
    assert!(ctx.recorded_pushes().is_empty());
}
...
fn test_pre_push_pr_create_failure() {
    ...
    assert_eq!(
        ctx.github().requests(),
        [vec![head_query(&ctx)], vec![testutil::GraphQlOperation::CreatePr],],
