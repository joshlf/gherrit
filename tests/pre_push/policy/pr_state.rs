...
fn non_open_pr_blocks_all_mutations() {
    ...
    assert_eq!(
        ...
        "one exact all-state local observation must reject the retired pull request"
    );
}

#[test]
fn terminal_pull_requests_for_one_change_are_reported_together_before_mutation() {
    let ctx = testutil::test_context!()
        .with_remote()
        .with_initial_commit()
        .with_mock_github()
    ...
    ctx.hook_cmd("pre-push")
        ...
        .stderr(predicates::str::contains(format!(
            "Cannot push GHerrit change '{id}' because PR #7 is closed"
        )))
        .stderr(predicates::str::contains(format!(
            "Cannot push GHerrit change '{id}' because PR #9 is merged"
        )));
