...
fn test_version_increment() {
    ...
    // Verify v1 pushed
    let v1_destination = format!("refs/tags/gherrit/{gherrit_id}/v1");
    let v1_count = ctx
        .successful_ref_updates()
        .iter()
        .filter(|update| update.destination == v1_destination)
        .count();
    assert_eq!(v1_count, 1, "Expected exactly one v1 tag update");
    ...
    // Verify v2 pushed
    let v2_destination = format!("refs/tags/gherrit/{gherrit_id}/v2");
    let v2_count = ctx
        .successful_ref_updates()
        .iter()
        .filter(|update| update.destination == v2_destination)
        .count();
    assert_eq!(v2_count, 1, "Expected exactly one v2 tag update");

    // Verify v1 NOT pushed AGAIN.
    let v1_count_final = ctx
        .successful_ref_updates()
        .iter()
        .filter(|update| update.destination == v1_destination)
        .count();
    ...
    assert!(ctx.remote_ref_oid(&v1_destination).is_some(), "Remote should contain v1 tag");
    assert!(ctx.remote_ref_oid(&v2_destination).is_some(), "Remote should contain v2 tag");
}
