...
fn test_full_stack_lifecycle_mocked() {
    ...
    // complex in a test env.
    // Verify valid sync
    testutil::assert_snapshot!(ctx, ctx.gherrit().args(["hook", "pre-push"]), "full_stack_lifecycle_push");
...
fn test_install_command_edge_cases() {
    ...
    testutil::assert_snapshot!(ctx, ctx.gherrit().args(["install", "--force"]), "install_edge_cases_force");
    ...
    testutil::assert_snapshot!(ctx, ctx.gherrit().args(["install"]), "install_edge_cases_idempotent");
...
fn test_install_configuration_and_security() {
    ...
    testutil::assert_snapshot!(ctx, ctx.gherrit().args(["install"]), "install_security_custom_internal");
...
fn test_manage_drift_detection() {
    ...
    testutil::assert_snapshot!(ctx, ctx.gherrit().args(["manage", "--private"]), "manage_drift_init");
    ...
    testutil::assert_snapshot!(ctx, ctx.gherrit().args(["manage", "--public"]), "manage_drift_attempt_switch");
...
fn test_manage_toggle_visibility() {
    ...
    // 1. Private
    testutil::assert_snapshot!(ctx, ctx.gherrit().args(["manage", "--private"]), "manage_toggle_init_private");
    ctx.assert_config("branch.visibility-feature.pushRemote", Some("."));

    // 2. Public
    testutil::assert_snapshot!(ctx, ctx.gherrit().args(["manage", "--public"]), "manage_toggle_switch_public");
...
fn test_post_checkout_drift_detection() {
    ...
    // Switch to it
    testutil::assert_snapshot!(ctx, ctx.git().args(["checkout", "drift-stack"]), "post_checkout_drift_stack");
}
