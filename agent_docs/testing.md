...
- validates the production endpoint and authenticated GitHub destination
  before remote I/O, including for an empty stack, so GitHub's absent-or-direct
  head and tag contract governs every exact ref observation; custom test
  destinations construct only refs with the same property;
- observes an optional public branch under that provider or fixture contract
  in the same initial request, then repeats the exact named default in the
  first exact-local query for a nonempty stack;
...
- remote Git observation constructs exact logical evidence for the default and
  local change namespaces plus the configured public branch under the
  GitHub.com or trusted-fixture absent-or-direct ref contract, while safely
  bounding and discarding unrelated records which Git's broader advertisement
  may return;
- authoritative absence, immutable history, and annotated marker identity
  survive byte parsing and bounded object decoding without treating generic
  `ls-remote` omission as proof;
...
projection stage made of one or more bounded GraphQL requests which close
...
effects in order
  ...
  draft-conversion specifications and receipts
  initial Git refs and leases (tuples plus optional public branch)
  create specifications and receipts
  marker refs and leases
  close receipts and update patches
...
separate, visible effect at the end of the initial Git stage. The optional
`refs/tags/gherrit/G/pr` marker is another separate, later create-only annotated
tag which peels to `v1` and records the canonical pull request number; a
...
- every unmarked contender and noncanonical duplicate must be draft on its
  owned base with landing automation disabled;
...
1. Empty private local intent and an already-current empty public projection
   validate endpoint/destination compatibility, then exit after the initial
   symbolic-`HEAD`/public observation with no exact-local read, token access, or
   GitHub request. An absent or divergent empty public projection first
   completes exact named-default observation, then plans and writes only its
   public branch without GitHub authentication.
