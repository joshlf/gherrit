...
- derives the intended ordered local stack from that retained tip and the
  candidate default name and tip supplied by the initial symbolic-`HEAD`
  observation, without resolving the live worktree branch again;
- observes an optional public branch in that same initial request, then repeats
  the exact named default in the first exact-local query for a nonempty stack;
- completes that exact named-default observation before planning or writing an
  absent or divergent empty public projection, while an empty private stack and
  an already-current empty public projection add no second read;
- publishes the public projection for an empty public stack before returning,
  without GitHub authentication or requests;
- keeps planner evidence scoped to the exact default and optional public refs
  plus Git and GitHub state belonging to local change IDs;
...
- uses the marker-bound validated same-repository OPEN row as canonical, closes
  every other validated same-repository OPEN row, and never promotes a
  duplicate merely because of number ordering;
- lets the lowest unmarked validated same-repository OPEN number contend
  deterministically for the marker lease without treating it as canonical
  first;
- rejects landing automation on every row which is or will become based on an
  owned base before exposing any mutation, then converts a marker-bound ready
  root to draft before any managed Git ref change whenever its desired stack
  position is nonroot, and never marks a pull request ready automatically;
...
- remote Git observation constructs exact logical evidence for the default and
  local change namespaces plus the configured public branch, while safely
  bounding and discarding unrelated records which Git's broader advertisement
  may return;
...
- GitHub exhausts one OPEN-only connection for every exact local ID, with one
  HTTP request able to alias several pages and one connection able to require
  several one-row page requests;
- GraphQL aliases, cursors, errors, nullable fields, and partial mutation
  outcomes are decoded conservatively;
- every mutation receipt has the exact alias set and echoed client mutation ID:
  conversions return the same OPEN draft and topology, creates return a new
  exact same-repository OPEN draft and tuple, closes return the same CLOSED
  identity, and final updates return the same OPEN identity without asserting
  draft state;
...
remote symbolic HEAD + exact public ref --/         |
                                                    ...
                                                    +-> nonempty
                                                            ...
                                                            -> exact local OPEN PR connections ----------+
                                                                                                        |
                                                                                              publication plan
                                                                                                        |
...
publication. Exact acknowledgement of all required initial Git batches—including
the optional public effect—releases pull request creation. An initially observed
validated contender supplies one marker template; a newly created pull request
supplies its marker identity only through an exact create receipt. When any
create is required, every marker template remains in one preplanned stage until
all create receipts and the receipt-dependent projection pass preflight. Exact
acknowledgement of every marker push releases one final projection stage made of
one or more bounded GraphQL requests which close noncanonical rows and update
canonical rows. If an earlier tuple batch succeeds but the final
public-containing batch fails atomically, the earlier tuples remain durable and
a fresh attempt plans only the remaining work.
...
trigger observation or replanning. An indeterminate write ends the attempt. The
same attempt does not retry the mutation or roll back. A fresh process
reconstructs all authority from durable Git and GitHub evidence.
...
same planner input—marker present with its numbered OPEN row absent—and fails
closed. A visible duplicate cannot replace it. A terminal duplicate needs no
closure, while an unmarked terminal row has no authority and does not defeat
sealed OPEN absence. Production neither downloads terminal history nor
...
subset, ends the modeled attempt, discards all process-local authority, starts
a new modeled process with its own fresh observation, and requires the next
...
3. Amendment and reorder: the marker remains the same annotated tag on `v1`
   ...
   or remarking an established pull request; every published-head ×
   published-first-parent stale OPEN pairing remains safe; and a marker-bound
   ready root becoming nonroot crosses the draft-conversion barrier before any
   Git tuple changes only after landing automation has been rejected. Either
   landing flag rejects before conversion, and a stale second conversion is
   indeterminate rather than an already-draft acknowledgement.
4. Visibility: hiding an unmarked provisional OPEN row repeats only the
   ...
   it, and a later fresh invocation with complete visibility restores its
...
- arbitrary subsets of complete nullable mutation aliases, with every mutation
  request transmitted exactly once and every invalid receipt ending the
  attempt.
...
2. The strict GitHub fake validates schema-conforming requests, applies
   ...
   closes any exact OPEN identity, and records ordered requests. It may store
   terminal rows for the independent durable oracle, but its production query
   surface returns and paginates only OPEN rows. This service model is
   deliberately weaker than GitHub's current base-sensitive refusal.
...
effects in order
  exact observations
  physical GraphQL request boundaries, aliases, and count
...
separate, visible effect at the end of the initial Git stage. The separately
optional `refs/tags/gherrit/G/pr` marker is a later create-only annotated tag
which peels to `v1` and records the canonical pull request number; a
zero-version history requires that marker and both mutable refs to be absent.
Required draft conversions form an earlier GraphQL barrier. Pure tests cover
every meaningful durable prefix and exact complete-alias subset, end the
modeled attempt, discard its authority, and construct a new process observation
before planning remaining work.
...
- a sealed absent OPEN connection plus marker absence is the only create
  authority, whether published history is absent or nonempty;
- a marker with absent published history is invalid history and is rejected
  before a pull request planner value exists;
- every retained same-repository OPEN row passes history, base, draft, and
  landing-state validation before any mutation is exposed;
- every published head × published owned-base first-parent observation is
  accepted independently, proposal-only and otherwise unpublished object IDs
  are rejected, and every published-plus-proposal head/base pairing satisfies
  the historical reachability relation;
...
- without a marker, the lowest validated same-repository OPEN number is only a
  deterministic contender;
- a marker whose exact number is absent from the OPEN set fails closed;
- terminal rows exist only in the independent oracle: terminal duplicates are
  absent from production observation and require no closure, an unmarked
  terminal row has no authority, and a marker-bound terminal row is
  indistinguishable from an omitted row and fails closed;
- canonical draft-owned, draft-default, and ready-default states are accepted
  only under the base/landing matrix: landing automation is invalid whenever
  the observed or desired base is owned and is allowed only for a canonical
  which remains on the default branch; ready-owned is rejected before a
  validated planner value exists;
- only a marker-bound ready-default canonical whose desired base is owned and
  whose landing state is inert requires the draft-conversion barrier;
- landing automation on that departing ready root rejects before conversion or
  any other write, and a stale second conversion is indeterminate rather than
  an idempotent acknowledgement that the row is already draft;
- an unexplained OPEN head is rejected;
- every create uses `G` and `gherrit-bases/G` with `draft: true`;
- every validated noncanonical same-repository OPEN identity becomes an exact
  closure after the marker barrier; and
...
and marker refs, peeled targets, strict tag bytes, and object IDs. Lost-ack
tests perform a real successful atomic push and replace or corrupt only its
acknowledgement. They prove that each batch leaves a complete atomic-unit
prefix, that earlier tuple batches may remain after a later public-containing
batch fails, and that no GitHub action
...
The scripted transport proves exact local OPEN-only one-row queries, complete
independent pagination and raw row/page budgets, repository/default agreement,
fork filtering only after budget accounting, exact mutation documents and the
four distinct receipt contracts, and mixed exact duplicate-closure and minimal
canonical-update projections. It asserts physical request boundaries and counts
without confusing a logical connection, page, alias, or projection stage with
one HTTP request.
...
- omission of the marker-bound canonical row never authorizes its closure or
  replacement, while a later fresh invocation with complete visibility restores
  it and closes every visible noncanonical same-repository row.

Large pull request populations with other head names must leave the GraphQL
documents, physical request count, local evidence set, planner result, and
mutation trace unchanged.
Cross-repository rows which reuse a requested local head name are followed and
discarded only within the per-ID baseline plus shared raw-row budget; they may
add pages and physical requests. Terminal rows add neither.

### Complete-process composition

Retained process scenarios prove:

1. Empty private local intent and an already-current empty public projection
   exit after the initial symbolic-`HEAD`/public observation with no exact-local
   read, token access, or GitHub request. An absent or divergent empty public
   projection first completes exact named-default observation, then plans and
   writes only its public branch without GitHub authentication.
...
4. Incomplete tuples, unsafe bases, and landing automation on any row which is
   or will become owned reject before every mutation, including a ready-root
   draft conversion.
5. Lost draft-conversion, tuple, public, create, marker, and final-projection
   acknowledgements leave safe durable prefixes. The current attempt stops;
   fresh invocations converge using their normal initial observations, without
   same-attempt confirmation reads, replanning, or mutation retries.
6. Exact local Git and GitHub reads overlap after stack derivation. No
   repository-wide or nonlocal state enters planner evidence; bounded unrelated
   Git advertisement records and cross-repository GitHub rows are discarded at
   their adapters.
...
Complete readable snapshots show results, physical GraphQL requests, traces,
refs, and pull requests.
Structural assertions remain mandatory for literal object IDs, tuple
atomicity, barrier order, exact request scopes and counts, and absence of
writes.
