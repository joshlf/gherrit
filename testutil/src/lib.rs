...
pub struct RefUpdate {
    /// `None` represents a deletion refspec with an empty source.
    pub source: Option<String>,
    pub destination: String,
}

impl RefUpdate {
    fn from_refspec(refspec: &str) -> Self {
        ...
        let (source, destination) = match refspec.split_once(':') {
            ...
            // A fully-qualified colonless ref pushes to the same destination.
            None => (Some(refspec), refspec),
        };
        Self { source: source.map(ToString::to_string), destination: destination.to_string() }
    }
}
...
impl TestContext {
            ...
            state
                ...
                .map(|push| PushRecord {
                    ...
                    ref_updates: push
                        .refspecs
                        .iter()
                        .map(|refspec| RefUpdate::from_refspec(refspec))
                        .collect(),
    ...
    pub fn successful_ref_updates(&self) -> Vec<RefUpdate> {
        self.recorded_pushes()
            .into_iter()
            .filter(PushRecord::succeeded)
            .flat_map(|push| push.ref_updates)
            .collect()
    }
...
mod tests {
    ...
    fn normalizes_push_refspec_forms() {
        assert_eq!(
            RefUpdate::from_refspec("+HEAD:refs/heads/main"),
            RefUpdate {
        ...
        assert_eq!(
            RefUpdate::from_refspec("refs/heads/main"),
            RefUpdate {
        ...
        assert_eq!(
            RefUpdate::from_refspec(":refs/heads/old"),
