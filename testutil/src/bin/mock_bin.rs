...
fn handle_git(args: &[String]) {
    // Check if we have the server URL. If not, fallback to just running git (shouldn't happen in test ctx)
    let server_url = match env::var("GHERRIT_MOCK_SERVER_URL") {
        Ok(url) => url,
        Err(_) => {
            // Fallback for when run outside of TestContext control (e.g. manual debugging?)
            // Or maybe pure passthrough.
            run_real_git(args);
            return;
        }
    };
