...
impl PrEntry {
    pub fn mock(id: u64, title: String, body: String, head: String, base: String) -> Self {
        ...
        let url_path =
            format!("repos/{}/{}/pulls/{}", crate::DEFAULT_OWNER, crate::DEFAULT_REPO, id);
    ...
    for selection in field.selection_set.selections.iter() {
        if let executable::Selection::Field(sub_field) = selection {
            match sub_field.name.as_str() {
                "pullRequests" => {
                    let head = sub_field.arguments.iter().find_map(|arg| {
                        match (&*arg.name, &*arg.value) {
                            ("headRefName", ast::Value::String(s)) => Some(s.as_str()),
                            _ => None,
                        }
                    });
                ...
                "id" => {
                    repo_data.insert("id".to_string(), serde_json::json!("REPO_NODE_ID"));
                }
                _ => {}
            }
        }
