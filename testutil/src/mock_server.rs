...
async fn list_prs(State(state): State<AppState>) -> Json<Vec<PrEntry>> {
...
fn read_state(path: &PathBuf) -> MockState {
    // Use fs::read_to_string directly. Since tests run serially or with separate temp dirs,
    // lock contention shouldn't be a major issue for now, but we can add fs2/gix lock if needed.
