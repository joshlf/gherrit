...
async fn graphql(
    ...
    for caps in update_re.captures_iter(query) {
        let idx = caps.name("idx").unwrap().as_str();
        let node_id = caps.name("id").unwrap().as_str();
    ...
    for caps in create_re.captures_iter(query) {
        ...
        let title_val = serde_json::from_str::<String>(title).unwrap();
        let body_val = serde_json::from_str::<String>(body).unwrap();
