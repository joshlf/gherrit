...
async fn graphql(
    ...
    // Regex definitions
    // Note: These regexes assume the client sends specific formatting (which common logic ensures).
    // We use .? for non-greedy matching of body content.
    let update_re = re!(
    ...
    for caps in update_re.captures_iter(query) {
        ...
        // Unescape body if needed? serde_json to_string escapes it. Regex matches raw chars.
        ...
        if let Some(pr) = mock_state.prs.iter_mut().find(|p| p.node_id == node_id) {
            // We should ideally unescape the strings, but for tests we simplify
            if let Ok(val) = serde_json::from_str::<String>(&format!("\"{}\"", title)) {
                pr.title = Some(val);
            } else {
                pr.title = Some(title.to_string());
            }
            if let Ok(val) = serde_json::from_str::<String>(&format!("\"{}\"", body)) {
                pr.body = Some(val);
            } else {
    ...
    for caps in create_re.captures_iter(query) {
        ...
        let mut title_val = title.to_string();
        if let Ok(val) = serde_json::from_str::<String>(&format!("\"{}\"", title)) {
            title_val = val;
        }
        let mut body_val = body.to_string();
        if let Ok(val) = serde_json::from_str::<String>(&format!("\"{}\"", body)) {
            body_val = val;
        }
