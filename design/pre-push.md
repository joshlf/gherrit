...
durable prefix only while the earliest same-repository review remains OPEN:
the lowest-numbered row is canonical, every higher OPEN row is closed after
the marker barrier, and only the canonical row receives the final projection.
A lower CLOSED row or any MERGED row means the change has terminal history and
rejects a later attempt rather than promoting an OPEN duplicate. Higher CLOSED
rows are the durable result of duplicate cleanup and are ignored. Even for the
same durable revision, final projections may differ because navigation reflects
each publisher's complete local stack. Those differences affect text only;
root status and every allowed base follow immutable ancestry. Competing
projections are therefore safe
...
- one fully paginated all-lifecycle pull request connection for every local
  change ID;
...
the exact head name and every lifecycle state:
...
Every connection is exhausted so all visible same-repository lifecycle rows
participate in one decision. This is important when an earlier canonical pull
request has become CLOSED or MERGED while a higher duplicate remains OPEN. An
OPEN-only query would promote the duplicate and could publish a review for a
change which has already ended.
...
validates and registers its coupled pull request number and node ID before its
lifecycle is folded into the connection.

After exhaustion, no same-repository row yields an opaque sealed `Absent`
value. Otherwise the lowest pull request number identifies the canonical
review history. The attempt rejects if that row is CLOSED, if any row is
MERGED, or if no row remains OPEN. When the lowest row is OPEN and every
terminal row is a higher CLOSED duplicate, the OPEN rows form the planner's
ordered set: the lowest is canonical and the higher OPEN identities are
repairable duplicates. Higher CLOSED rows are already-repaired duplicates and
are discarded. `Absent` means only that this exact connection returned no
same-repository row; it does not assert snapshot isolation or timeless
absence.
...
only the canonical identity, base kind, title, and body plus each duplicate's
exact identity; head and base object IDs and landing state cannot enter effect
planning. Cross-repository rows and admissible higher CLOSED duplicates discard
their validated payload. No repository-wide or nonlocal identity collection is
...
- one sealed `Absent` or a nonempty validated OPEN set with its canonical row
  and higher-numbered duplicates, after invalid lifecycle history has been
  rejected; and
...
| any | any | any MERGED row, or a lowest-numbered CLOSED row | Reject before planning |
...
The lifecycle fold establishes that the lowest same-repository row is OPEN,
that no row is MERGED, and that every admissible CLOSED row has a higher
number. The lowest OPEN row is therefore the canonical identity independently
of pagination order. Every higher OPEN row is scheduled for exact closure,
and only the canonical row's title, body, and base participate in final
projection. A stale observation may miss a lower row and temporarily select a
higher one, but it can never close an unseen lower identity. Because GitHub
numbers later creations above earlier ones, a later complete observation can
only keep the selected canonical row or replace it with a lower row and close
the former selection. Update receipts additionally require that the selected
canonical remains OPEN before the attempt accepts its projection.
...
form one preplanned atomic batch. This includes markers justified by observed
OPEN pull requests and markers whose authority depends on create receipts. The
complete batch remains inaccessible until every create receipt is validated
and the receipt-dependent projection is preflighted. This deliberately delays
already-justified markers rather than representing two independently releasable
sets. It is one staged plan, not a second observation or a replan.
...
reconstructs durable state through the complete all-lifecycle observation.
...
Every exact lifecycle connection is exhaustive over its returned cursors, but
...
If a marked pull request is omitted, creation is forbidden. Invalid terminal
...
which participated in `includeIf.hasconfig:remote.*.url` selection. Git does
not rescan those conditions after GHerrit's later `--config-env` options add
the private remote. The adapter therefore chooses a name absent from the
observed finite configuration and configures it with the exact URL and push URL
plus the effective `remote.<configured>.proxy` and
...
The push child sets `LC_ALL=C`, overriding the inherited message locale before
Git renders the otherwise human-readable header and footer used to frame that
receipt. Receipt parsing therefore does not depend on the user's locale.
...
agree. It exposes one exact ordered local observation only after every
all-lifecycle connection is exhausted.
...
invalid exact lifecycle history
...
visible lifecycle history for those IDs and with the literal commit graph
reachable from their relevant published versions. A single local ID can
therefore still require substantial work, but the row budget bounds it. Reading
CLOSED history is necessary to prove that an earlier canonical review was not
replaced by a higher OPEN duplicate. Pull requests with other head names do not
enter the attempt. Cross-repository pull requests which reuse an exact local
head name can enter the bounded raw response, but are discarded before they
become identity or planning evidence.
...
- one logical all-lifecycle GraphQL observation for the exact local IDs; and
...
confirmation query, rollback, or same-attempt re-observation. Git advertisement
breadth is the explicit transport boundary described under exact local Git
evidence.
...
- complete independently paginated all-lifecycle queries for exact local IDs;
- sealed absence, one OPEN, terminal-only history, lower terminal canonicals,
  higher CLOSED duplicates, canonical selection among duplicate OPEN rows,
