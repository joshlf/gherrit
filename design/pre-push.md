...
There is no repository-wide OPEN scan, terminal history on the established
path, nonlocal tag observation, nonlocal object acquisition, marker
confirmation query, rollback, or same-attempt re-observation.
...
- observation is scoped to exact local head names and bounded even when forks
  reuse those names: for `N` IDs and `K <= N` terminal probes, pull request
  pagination accepts at most `N + K + 99` raw rows and `2N + 2K + 99` pages,
  while Git validation additionally depends on the distinct external
  published-version ancestry reachable from those IDs.
