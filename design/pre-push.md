...
- an absent marker or the exact canonical pull request number decoded from a
  validated immutable marker;
...
| any | pull request `N` | no row numbered `N` | Fail closed |
| absent | any | one or more | Reject unexplained pull request state |
...
Every local change whose validated OPEN set lacks a marker requires one marker
operation. An observed contender supplies its number during planning. A newly
created contender supplies its number only when its exact receipt is consumed.
...
its receipt must return that same number and node ID in OPEN state with the
same draft state the update was required to preserve.
