...
prove presence or defeat authoritative absence. Peeled records, malformed
records, and records in an owned but unrequested namespace reject.
...
continues past them. Same-repository row identities are registered and their
relevant states are folded as the pages arrive, but the correlated result
remains unavailable until the exact connection is exhausted:
...
timeouts and incomplete cleanup, expose no child output.
...
ID can therefore require substantial work, but unrelated repository history
does not enter the attempt.
...
- observation is scoped to local change IDs rather than unrelated repository
  state: for `N` IDs, pull request pagination accepts at most `N + 99` rows and
  `2N + 99` pages, while Git validation additionally depends on the distinct
  external published-version ancestry reachable from those IDs.
