...
- Git and the user's trusted transport extensions deliver destination-bound
  commands to the service named by that resolved destination;
- GitHub.com represents every `refs/heads/*` name as absent or a direct commit
  reference and every `refs/tags/*` name as absent or a direct object
  reference, never as a symbolic reference;
...
GHerrit then resolves the configured Git push destination and validates it
against the selected GitHub endpoint before performing any remote I/O. The
production endpoint requires an HTTPS or SSH transport to `github.com`; an
explicit custom or disabled test-driver endpoint may use a local or custom Git
destination. A disabled endpoint can execute only a plan which needs no GitHub
operation. This gate applies even when the local stack will be empty.

The first remote request observes the symbolic target name of `HEAD` and the
direct `HEAD` object ID. In public mode the same request also observes the
exact public branch or its absence. The symbolic name and direct object provide
the candidate default branch from which GHerrit derives the local stack. They
do not prove that the named branch itself was advertised at that object.
...
Endpoint compatibility is a local check. No empty-stack path reads a GitHub
token, constructs an authenticated client, or sends a GitHub request. Unrelated
GitHub availability and credentials are therefore irrelevant.
...
The first remote Git read runs `ls-remote --symref` and asks for symbolic
`HEAD`, the direct `HEAD` object, and, in public mode, the exact public branch.
The public branch adds a ref pattern but no network round trip. The direct
`HEAD` object does not by itself prove that the symbolic target exists by its
full name at that object.

Git transport output cannot prove for an arbitrary server that a requested
head or tag is direct or absent. Older wire protocols can render a symbolic
ref as only its resolved object ID. A smart-HTTP helper can silently fall back
despite a client request for protocol v2, and even a genuine v2 server omits a
dangling symbolic ref whose target does not exist. GHerrit therefore does not
infer generic direct-ref or absence guarantees from `ls-remote` output.

The production interpretation instead rests on the service boundary proved
before this read. GitHub [defines a branch as a Git reference which stores a
commit hash](https://docs.github.com/en/rest/git/refs#about-git-references), and
its reference API represents every returned ref with a direct object type and
SHA. After the destination check selects GitHub.com under the trusted Git
transport boundary, a requested head or tag is therefore either absent or
direct under the service contract. A visible symbolic record is still rejected
defensively. Custom and disabled endpoints are feature-gated test facilities;
their Git destinations are trusted fixtures which must provide the same
absent-or-direct property rather than a production safety boundary.
...
These authoritative absence claims use the same GitHub service or trusted
fixture contract as the initial public-branch observation; omission from
generic `ls-remote` output would not be sufficient by itself.
...
capture local branch, management intent, and HEAD
    ...
    -> resolve destination
    -> validate endpoint and destination compatibility
    -> observe symbolic HEAD target, direct HEAD object, and optional exact
       public branch under the selected service contract; derive local stack
...
destination. Immediately after resolution, GHerrit validates the destination
...
unauthenticated `git` protocol is disabled. URI scheme spelling must be lower
case; the host name is compared without ASCII case distinctions. Local
filesystem paths, other hosts, insecure or custom schemes, helper aliases, and
case-variant schemes are rejected before remote I/O. This prevents a custom
`git-remote-*` helper or another repository from handling Git traffic while
the API client acts on the production GitHub repository.

Custom and disabled endpoints exist only in the test driver and may be paired
with local filesystem paths or custom Git transports. The disabled endpoint
can execute only plans which need no GitHub operation. A scheme-less filesystem
destination is resolved relative to the repository context used by Git and
canonicalized once. A missing or non-UTF-8 canonical target is rejected without
rendering the path. `file://` URLs remain unsupported; tests which need a local
repository use its native filesystem path.
...
completes observation before planning. Every empty path performs endpoint
compatibility validation but zero GitHub authentication and zero GraphQL
requests. Destination and configuration inspection uses local processes and
does not add a remote request.
...
- exact local Git ref and tag observation, including authoritative absence
  under the provider or fixture contract;
- production endpoint and destination rejection before every remote read,
  including empty-stack paths and insecure GitHub transports, while custom and
  disabled test-driver endpoints admit local and custom Git destinations
  within their operation limits;
- exact public branch presence and absence under the documented GitHub service
  contract, defensive rejection of visible symbolic records, malformed record
  rejection, and construction of custom fixtures with only absent or direct
  requested refs;
