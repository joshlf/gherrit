...
destination. It configures that name with the exact URL and push URL plus the
effective `remote.<configured>.proxy` and
`remote.<configured>.proxyAuthMethod` values, when present. The private values
travel through dedicated environment variables and never through arguments.
Final validation proves that the generated remote has exactly those planned
keys and values, and a URL-only probe proves that another `insteadOf` rule
cannot rewrite it.
